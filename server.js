  ...
  const fallbackAssetLinks = [
    {
      ...
      target: {
        ...
        sha256_cert_fingerprints: [
          ...
          "A4:85:94:E2:20:47:77:FA:62:4B:9C:07:FD:F5:9E:34:66:D0:2C:F0:D9:1E:FB:CE:3F:74:68:89:F0:61:A1:8B"
        ]
      }
    }
  ];
...
function normalizeRuntimeTraceEvents(input) {
  const safeEvents = Array.isArray(input) ? input : [];
  return safeEvents
    .slice(-220)
    .map((raw) => {
      ...
      const stage = typeof raw.stage === "string" ? raw.stage.trim().slice(0, 80) : "";
      if (!stage) {
        return null;
      }
      ...
      const booleanFields = [
        "atBottomReply",
        "usedStreaming",
        "shouldRenderInCurrentChat",
        "hasReply",
        "canSnapOnCreate",
        "force",
        "interrupted",
        "finalizedStreamInPlace"
      ];
      ...
      const stringFields = ["conversationId", "screenId"];
      for (const key of stringFields) {
        if (typeof raw[key] === "string" && raw[key].trim()) {
          event[key] = raw[key].trim().slice(0, 120);
        }
      }
...
app.post("/chat/runtime-trace", (req, res) => {
  const requestId = typeof req.body?.requestId === "string" ? req.body.requestId.trim() : "";
  const conversationId = typeof req.body?.conversationId === "string" ? req.body.conversationId.trim() : "";
  const reason = typeof req.body?.reason === "string" ? req.body.reason.trim() : "";
  const transport = typeof req.body?.transport === "string" ? req.body.transport.trim() : "unknown";
  const events = normalizeRuntimeTraceEvents(req.body?.events);

  if (!requestId) {
    return res.status(400).json({ error: "Missing requestId" });
  }

  const runtimeLogger = childLogger({
    scope: "chat_runtime_trace",
    requestId,
    conversationId: conversationId || null
  });

  const compactTimeline = events
    .map((event) => {
      const st = Number.isFinite(event.scrollTop) ? event.scrollTop : "?";
      const db = Number.isFinite(event.distanceBottom) ? event.distanceBottom : "?";
      const ll = Number.isFinite(event.liveLength) ? event.liveLength : "?";
      return `${event.stage}(top:${st},dist:${db},len:${ll})`;
    })
    .join(" -> ")
    .slice(0, 6000);

  runtimeLogger.info({
    event: "client_runtime_trace",
    reason: reason || null,
    transport,
    eventCount: events.length,
    firstStage: events[0]?.stage || null,
    lastStage: events[events.length - 1]?.stage || null,
    timeline: compactTimeline,
    events
  });

  return res.json({ success: true, requestId, eventCount: events.length });
});
...
// Pragmatic SSE version of /chat - MVP implementation.
app.post("/chat/stream", async (req, res) => {
  if (appConfig.enableChatStreaming !== true) {
    return res.status(405).json({ error: "Chat streaming is not enabled", code: "streaming_disabled" });
  }
  ...
  // MVP: placeholder for Phase 2 full cloning
  try {
    res.write(`data: ${JSON.stringify({error: "Chat streaming MVP - full implementation pending"})}\n\n`);
    res.end();
  } catch (err) {
    res.write(`data: ${JSON.stringify({error: "Chat streaming failed"})}\n\n`);
    res.end();
  }
});
