...
const USER_AGENT =
  'facilitatio-emergency-numbers-updater/1.0 (facilitat.io; contact via maintainer)';
 ...
 */
async function updateEmergencyNumbers(
  outputFile,
  logPrefix = '[emergency-updater]'
) {
  ...
  try {
    ...
    console.warn(
      `${logPrefix} Fichier actuel introuvable ou invalide, départ depuis zéro.`
    );
  }
  ...
  try {
    ...
    console.log(
      `${logPrefix} ${Object.keys(emergencyMap).length} pays avec numéros d'urgence.`
    );
  } catch (err) {
    console.error(`${logPrefix} Échec requête urgences:`, err.message);
    console.warn(
      `${logPrefix} Conservation des données existantes pour les urgences.`
    );
  }
  ...
  try {
    ...
    console.log(
      `${logPrefix} ${Object.keys(suicideMap).length} pays avec lignes suicide.`
    );
  } catch (err) {
    console.error(`${logPrefix} Échec requête suicide:`, err.message);
    console.warn(
      `${logPrefix} Conservation des données existantes pour les lignes suicide.`
    );
  }
  ...
  const totalCountries = Object.keys(updated).filter(
    (k) => !k.startsWith('_')
  ).length;
  console.log(
    `${logPrefix} Terminé. ${totalCountries} pays au total. ${newEntries} nouveaux, ${updatedEntries} mis à jour.`
  );
