...
function createMemoryHelpers({
  ...
  async function updateMemory(previousMemory, history, promptRegistry = buildDefaultPromptRegistry(), memoryPrioritySignal = "normal", theoreticalOrientationSignal = "none", intersessionMemoryCompressed = "", limitingBeliefValidated = false) {
    ...
    // Si limiting_belief n'est pas validé par C3 (confiance insuffisante),
    // on n'expose pas ce signal à C4 pour éviter les faux positifs en mémoire.
    const effectiveOrientationSignal =
      theoreticalOrientationSignal === "limiting_belief" && !limitingBeliefValidated
        ? "none"
...
Orientation théorique détectée ce tour : ${effectiveOrientationSignal}
...
`;
  ...
  function shouldCompressMemoryCandidate(memoryCandidate = "", previousMemory = "") {
    ...
    // Capture uniquement jusqu'à la prochaine section (ex: "Lecture bot:") pour ne pas compter ses items
...
// Deterministic reset of the "Lecture bot" block in memory.
// Applied after updateMemory when the conversation state is not exploration_* or info_*.
// Replaces whatever follows "Lecture bot:" with a dash, regardless of what the LLM wrote.
function forceLectureBotReset(memoryText = "") {
  const text = String(memoryText || "");
  return text.replace(
    /(Lecture bot\s*:\s*)[\s\S]*/i,
    "$1\n- -"
  );
}
