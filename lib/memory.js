...
function createMemoryHelpers({
  ...
  function getPrimaryMemoryUpdateModel() {
    return MODEL_IDS.memoryUpdate || MODEL_IDS.analysis;
  }

  function getFallbackMemoryUpdateModel() {
    const fallbackModel =
      MODEL_IDS.memoryUpdateFallback || MODEL_IDS.generation || null;
  ...
  async function runMemoryCompletionForModel(
    ...
    const shouldRetryAfterLengthExhaustion =
      !raw &&
      String(response?.choices?.[0]?.finish_reason || '').toLowerCase() ===
        'length';
