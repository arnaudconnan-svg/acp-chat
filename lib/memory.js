...
const MAX_INTERSESSION_MEMORY_CHARS = 6000;

function normalizeIntersessionMemorySource(value = '') {
  return String(value || '')
    .replace(/\r\n/g, '\n')
    .split('\n')
    ...
    .replace(/\n{3,}/g, '\n\n')
    .trim()
    .slice(0, MAX_INTERSESSION_MEMORY_CHARS);
}

function normalizeIntersessionItem(value = '') {
  return String(value || '').replace(/\s+/g, ' ').trim();
}

function parseIntersessionMemoryItems(raw = '') {
...
function isManifestlyInterpretiveMemoryItem(item = '') {
  const normalized = normalizeIntersessionItem(item)
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '');
  return /\b(comme si|il semble|elle semble|semble que|peut etre|suggere|traduit|reflete|temoigne|montre que)\b/.test(
    normalized
  );
}

function validateIntersessionMemoryItems(raw = '') {
  const parsedItems = parseIntersessionMemoryItems(raw);
  ...
  for (const rawItem of parsedItems) {
    ...
    const item = normalizeIntersessionItem(rawItem);
    const key = item
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '');
    if (!item || isManifestlyInterpretiveMemoryItem(item) || seen.has(key)) {
      return null;
    }
    seen.add(key);
    items.push(item);
...
function normalizeMovementItem(
  value,
  fallbackNowIso = safeIsoString(Date.now())
) {
  ...
  const id =
    idRaw || buildMovementId({ text: text || 'vide', nowMs: Date.now() });
  return {
    id,
    text,
...
function normalizeMemoryStateShape(
  memoryState = null,
  _fallbackMemoryText = '',
  nowMs = Date.now()
) {
...
function renderMemoryTextFromState(memoryState = {}) {
  ...
  const stableLines =
    safe.sessionStableContext.length > 0
      ? safe.sessionStableContext.map((item) => `- ${item}`).join('\n')
      : '-';
  const ongoingLines =
    safe.onGoingMovements.length > 0
      ? safe.onGoingMovements.map((item) => `- ${item.text}`).join('\n')
      : '-';
  const ancientLines =
    safe.ancientMovements.length > 0
...
function mergeMemoryStateWithFinalizedText({
  ...
  const nowIso = safeIsoString(nowMs);
  const normalizedPrevious = normalizeMemoryStateShape(
    previousMemoryState,
    '',
    nowMs
  );
  const purgeResult = applyInactivityPurge(normalizedPrevious, {
    nowMs,
    lastActivityMs,
    ttlMs
  });
  ...
  const extractedStableContext = parseMemorySectionBullets(
    finalizedMemoryText,
    'Contexte stable'
  )
  ...
  const nextOngoingTexts = parseMemorySectionBullets(
    finalizedMemoryText,
    'Mouvements en cours'
  )
    .map(normalizeBulletText)
    .filter(Boolean);
  const nextOngoingUniqueTexts = dedupeDeterministicTexts(
    nextOngoingTexts,
    MAX_ONGOING_MOVEMENTS
  );
  ...
  if (deleteIds.length > 0) {
    ancientMovements = ancientMovements.filter(
      (item) => !deleteIds.includes(String(item.id || '').trim())
    );
  }
...
function _extractSectionBlock(memoryText = '', sectionLabel = '') {
  const label = String(sectionLabel || '').trim();
...
function extractItemsFromRawOutput(
  rawOutput = '',
  { maxItems = Infinity } = {}
) {
...
function extractDeleteAncientIdsFromRawOutput(rawOutput = '') {
  return parseJsonStringArrayField(rawOutput, [
    'deleteAncientIds',
    'deleteAncientMovementsById',
...
function buildMemoryTextForMerge({
  sessionStableContext = [],
  onGoingMovements = []
} = {}) {
  const stableLines =
    Array.isArray(sessionStableContext) && sessionStableContext.length > 0
      ? sessionStableContext.map((item) => `- ${item}`).join('\n')
      : '-';
  const ongoingLines =
    Array.isArray(onGoingMovements) && onGoingMovements.length > 0
...
function createMemoryHelpers({
  mistralTransport,
  MISTRAL_MODEL_IDS,
  normalizeIntersessionMemory: _normalizeIntersessionMemory,
  normalizeMemory
}) {
  function getPrimaryMemoryUpdateModel() {
    return MISTRAL_MODEL_IDS.memory;
  }

  function getFallbackMemoryUpdateModel() {
    const fallbackModel = null;
    const primaryModel = getPrimaryMemoryUpdateModel();
  ...
  function isMemoryFallbackCandidateError(err) {
  ...
  async function requestMemoryUpdateCompletionWithModel(
    model,
    system,
    user,
    maxCompletionTokens
  ) {
    return mistralTransport.complete({
      model,
  ...
  async function runMemoryCompletionForModel(
    model,
    system,
    userPayload,
    maxCompletionTokens
  ) {
    let response = await requestMemoryUpdateCompletionWithModel(
      model,
      system,
      userPayload,
      maxCompletionTokens
    ...
    const shouldRetryAfterLengthExhaustion =
      !raw &&
      String(response?.finishReason || '').toLowerCase() === 'length';
    if (shouldRetryAfterLengthExhaustion) {
      response = await requestMemoryUpdateCompletionWithModel(
        model,
        system,
        userPayload,
        MEMORY_UPDATE_RETRY_MAX_COMPLETION_TOKENS
      );
  ...
  async function updateMemory(
    ...
    const ongoingSystemPrompt = String(
      promptRegistry.EXTRACT_ONGOING_MOVEMENTS ||
        defaults.EXTRACT_ONGOING_MOVEMENTS ||
        ''
    ).trim();
    const stableSystemPrompt = String(
      promptRegistry.EXTRACT_STABLE_CONTEXT ||
        defaults.EXTRACT_STABLE_CONTEXT ||
        ''
    ).trim();
    const cleanupSystemPrompt = String(
      promptRegistry.CLEANUP_ANCIENT_DUPLICATES ||
        defaults.CLEANUP_ANCIENT_DUPLICATES ||
        ''
    ).trim();

    const previousNormalizedMemory = normalizeMemory(
      previousMemory,
      promptRegistry
    );
    ...
    async function runMemoryPrompt(promptName, system, userPayload) {
      const primaryModel = getPrimaryMemoryUpdateModel();
      const fallbackModel = getFallbackMemoryUpdateModel();
      const canFallback = Boolean(fallbackModel);
      ...
      try {
        result = await runMemoryCompletionForModel(
          primaryModel,
          system,
          userPayload,
          MEMORY_UPDATE_BASE_MAX_COMPLETION_TOKENS
        );
      } catch (err) {
        if (!canFallback || !isMemoryFallbackCandidateError(err)) {
          throw err;
        }

        console.warn('[MEMORY][MODEL_FALLBACK]', {
          prompt: promptName,
      ...
      if (!result.raw && canFallback && !usedFallbackModel) {
        console.warn('[MEMORY][MODEL_FALLBACK]', {
          prompt: promptName,
          reason: 'empty_output',
          fromModel: primaryModel,
      ...
      return {
        ...
        meta: {
          ...
          finishReason:
            typeof normalizedResponse?.finishReason === 'string'
          ...
          retriedAfterLengthExhaustion:
            result.retriedAfterLengthExhaustion === true
        }
      };
    }
...
`;
    ...
    const stablePromise = runMemoryPrompt(
      'extract_stable_context',
      stableSystemPrompt,
      stablePromptPayload
    );
    const ongoingResult = await runMemoryPrompt(
      'extract_ongoing_movements',
      ongoingSystemPrompt,
      ongoingPromptPayload
    );

    const extractedOngoing = extractItemsFromRawOutput(ongoingResult.raw, {
      maxItems: MAX_ONGOING_MOVEMENTS
    });

    let cleanupResult = { raw: '', meta: null };
    if (
      extractedOngoing.length > 0 &&
      Array.isArray(memoryState?.ancientMovements) &&
      memoryState.ancientMovements.length > 0
    ) {
      const ancientSnapshot = memoryState.ancientMovements
        .slice(-30)
        .map((item) => ({
...
`;
      ...
      cleanupResult = await runMemoryPrompt(
    ...
    const mergedStable = dedupeDeterministicTexts([
      ...previousStable,
      ...extractedStable
    ]);
    ...
    const deleteAncientMovementsById = extractDeleteAncientIdsFromRawOutput(
      cleanupResult.raw
    );
    ...
    return {
      ...
      llmMeta: {
        ...
        hasIntersessionMemory: Boolean(
          String(intersessionMemoryForTurn || '').trim()
        ),
        hasClinicalSignals: Boolean(
  ...
  async function updateIntersessionMemory(
    previousIntersessionMemory,
    sessionMemory,
    promptRegistry = buildDefaultPromptRegistry()
  ) {
    const defaultPrompt = String(
      buildDefaultPromptRegistry().UPDATE_INTERSESSION_MEMORY || ''
    ).trim();
    const currentPrompt = String(
      promptRegistry.UPDATE_INTERSESSION_MEMORY || ''
    ).trim();
...
`;
    ...
    const r = await mistralTransport.complete({
      model: MISTRAL_MODEL_IDS.memory,
      // Technical provider guard only: allows a source memory up to its 6000-char product budget.
      maxTokens: 2500,
      messages: [
        { role: 'system', content: system },
        { role: 'user', content: user }
    ...
    const items = validateIntersessionMemoryItems(r.content);
    if (!items) return previousSource;
    return items
      .map((item) => `- ${item}`)
      .join('\n')
      .slice(0, MAX_INTERSESSION_MEMORY_CHARS)
