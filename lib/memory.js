...
function mergeMemoryStateWithFinalizedText({
  ...
  // Deterministic safeguard: the same movement text must never be active and archived at once.
  // Keep ongoing as source of truth for the current turn and drop matching archived entries.
  if (sanitizedOngoingTexts.length > 0 && ancientMovements.length > 0) {
    const ongoingKeys = new Set(
      sanitizedOngoingTexts
        .map(text => canonicalMemoryTextKey(text))
        .filter(Boolean)
    );
    if (ongoingKeys.size > 0) {
      ancientMovements = ancientMovements.filter((item) => {
        const text = typeof item?.text === "string" ? item.text : "";
        const key = canonicalMemoryTextKey(text);
        return !key || !ongoingKeys.has(key);
      });
    }
  }
