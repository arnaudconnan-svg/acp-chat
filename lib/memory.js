...
// Archive onGoingMovements into ancientMovements when memoryUpdateDecision = "hold".
// This ensures that ongoing items from the previous turn do not persist indefinitely
// when the current turn does not justify a full LLM memory update.
function archiveOngoingMovementsOnHold({
  previousMemoryState = {},
  nowMs = Date.now(),
  lastActivityMs = null,
  ...
  if (base.onGoingMovements.length === 0) {
    return { memoryState: base, memoryText: renderMemoryTextFromState(base), archived: false };
  }
  ...
  const nextState = normalizeMemoryStateShape({
    sessionStableContext: base.sessionStableContext,
    onGoingMovements: [],
    ancientMovements: [...base.ancientMovements, ...archivedFromPrevious],
    pastSignals: base.pastSignals
  }, "", nowMs);
...
function mergeMemoryStateWithFinalizedText({
  ...
  if (sanitizedOngoingTexts.length > 0 && ancientMovements.length > 0) {
    const ancientKeys = new Set(
      ancientMovements
        .map((item) => canonicalMemoryTextKey(String(item?.text || "")))
        .filter(Boolean)
    );
    reactivatedItems = sanitizedOngoingTexts
  ...
  const onGoingMovements = sanitizedOngoingTexts.map((text, idx) => ({
    ...
    createdAt: nowIso,
    archivedAt: null,
    _reactivationGuardTriggered: reactivatedItems.includes(text) || false
  }));
...
`;
    ...
    let r = await requestMemoryUpdateCompletion(system, user, MEMORY_UPDATE_BASE_MAX_COMPLETION_TOKENS);
    let rawOutput = String(r.choices?.[0]?.message?.content || "").trim();

    // Technical safeguard: if reasoning consumed the budget and no user-visible content was produced,
    // retry once with a larger cap to recover a structured memory output.
    const shouldRetryAfterLengthExhaustion = !rawOutput && String(r?.choices?.[0]?.finish_reason || "").toLowerCase() === "length";
    if (shouldRetryAfterLengthExhaustion) {
      r = await requestMemoryUpdateCompletion(system, user, MEMORY_UPDATE_RETRY_MAX_COMPLETION_TOKENS);
      rawOutput = String(r.choices?.[0]?.message?.content || "").trim();
    }
