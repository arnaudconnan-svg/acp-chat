...
const MAX_INTERSESSION_MEMORY_ITEMS = 10;
const MAX_INTERSESSION_MEMORY_ITEM_LENGTH = 240;
...
function validateIntersessionMemoryItems(raw = '') {
  ...
  for (const rawItem of parsedItems) {
    ...
    if (
      !item ||
      item.length > MAX_INTERSESSION_MEMORY_ITEM_LENGTH ||
      isManifestlyInterpretiveMemoryItem(item) ||
      seen.has(key)
    ) {
      return null;
    }
    seen.add(key);
    items.push(item);
    if (items.length > MAX_INTERSESSION_MEMORY_ITEMS) return null;
  }
...
`;
  ...
  return {
    MEMORY_INACTIVITY_TTL_MS,
      MAX_INTERSESSION_MEMORY_ITEM_LENGTH,
      MAX_INTERSESSION_MEMORY_ITEMS,
