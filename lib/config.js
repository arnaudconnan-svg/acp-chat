...
const envSchema = z
  .object({
    ...
    MISTRAL_MODEL_ANALYSIS: z.string().trim().optional(),
    MISTRAL_MODEL_GENERATION: z.string().trim().optional(),
    MISTRAL_MODEL_MEMORY: z.string().trim().optional(),
    MISTRAL_MODEL_TITLE: z.string().trim().optional(),
    ...
    ENABLE_CHAT_STREAMING: booleanFromEnv(false),
    LOG_LEVEL: z
    ...
    LOG_PERSIST: booleanFromEnv(true),
    LOG_DIR: z.string().trim().default('logs'),
    LOG_RETENTION_DAYS: integerFromEnv(14),
    NOTIFY_EMAIL_TO: z.string().trim().optional(),
    HUMAN_RELAY_EMAIL_TO: z.string().trim().optional(),
    NOTIFY_SMTP_HOST: z.string().trim().default('smtp.hostinger.com'),
    NOTIFY_SMTP_PORT: integerFromEnv(465),
    NOTIFY_SMTP_SECURE: booleanFromEnv(true),
...
function inspectConfig(env = process.env) {
  ...
  const config = {
    ...
    mistralModelAnalysis: result.data.MISTRAL_MODEL_ANALYSIS || '',
    mistralModelGeneration: result.data.MISTRAL_MODEL_GENERATION || '',
    mistralModelMemory: result.data.MISTRAL_MODEL_MEMORY || '',
    mistralModelTitle: result.data.MISTRAL_MODEL_TITLE || '',
    ...
    enableChatStreaming: result.data.ENABLE_CHAT_STREAMING,
    logLevel: result.data.LOG_LEVEL,
    logPretty: result.data.LOG_PRETTY,
    logPersist: result.data.LOG_PERSIST,
    logDir: result.data.LOG_DIR,
    logRetentionDays: result.data.LOG_RETENTION_DAYS,
    notifyEmailTo: result.data.NOTIFY_EMAIL_TO || '',
    humanRelayEmailTo: result.data.HUMAN_RELAY_EMAIL_TO || '',
...
function summarizeConfig(config) {
  ...
  return {
    ...
    firebaseServiceAccountMode: safeConfig.firebaseServiceAccountPath
      ? 'path'
      : 'json',
    mistralConfigured: Boolean(safeConfig.mistralApiKey),
    branchRouteDebug: safeConfig.branchRouteDebug,
    devRuntimeGuards: safeConfig.devRuntimeGuards,
    enableChatStreaming: safeConfig.enableChatStreaming,
    logLevel: safeConfig.logLevel,
    logPretty: safeConfig.logPretty,
    logPersist: safeConfig.logPersist,
    logDir: safeConfig.logDir,
    logRetentionDays: safeConfig.logRetentionDays,
