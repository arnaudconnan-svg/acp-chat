...
function createAnalyzers({
  ...
  async function analyzeInfoSignal(message = "", history = [], promptRegistry = buildDefaultPromptRegistry()) {
    ...
    try {
      ...
      const detectedState = VALID_INFO_STATES.includes(rawState) ? rawState : "info_features";
...
`;
  ...
  async function analyzeContactSignal(
    ...
    if (!hasExclusion && contactMatch) {
      ...
      const contactExpression = firstMatchedPatternExpression(CONTACT_PATTERNS, message);
      const highCriticismExpression = firstMatchedPatternExpression(HIGH_CRITICISM_PATTERNS, message);
      return {
        isContact: true,
...
`;
  ...
  async function analyzeEmotionalDecentering(message = "", history = []) {
    ...
    const decenteringSecondHalfMatch = firstRegexMatch(DECENTERING_PATTERNS, secondHalf);
    const hasDecenteringInSecondHalf = typeof decenteringSecondHalfMatch === "string" && decenteringSecondHalfMatch.length > 0;
    ...
    const startsWithDecenteringMatch = String(message || "").match(/^\s*(bref|de toute façon|c'est pas grave|passons|oublie|laisse tomber)/i);
    const startsWithDecentering = Array.isArray(startsWithDecenteringMatch) && typeof startsWithDecenteringMatch[0] === "string";
...
`;
  ...
  function hasExplicitRelationalFriction(message = "") {
    ...
    return directCorrectionPatterns.some(p => p.test(text))
...
`;
    ...
    try {
      ...
      const allianceSignal = forcedRuptureFromHardSignal
        ? "rupture"
        : (parsedSignal === "good" && explicitRelationalFriction ? "fragile" : parsedSignal);
      ...
      return {
        allianceSignal: forcedRuptureFromHardSignal
          ? "rupture"
          : (explicitRelationalFriction ? "fragile" : "good"),
        explicitRelationalFriction,
        llmTriggered: true,
        source: "llm_fallback"
      };
  ...
  async function analyzeTechnicalContext(message = "") {
    const technicalContextDetected = shouldForceExplorationForSituatedImpasse(message) === true;
    ...
    return {
      ...
      source: technicalContextDetected ? "deterministic_human_field" : "deterministic_no_signal",
  ...
  async function analyzeRecallRouting({
    ...
    const historyText = recentHistory.map((entry) => String(entry?.content || "")).join(" \n ");
    ...
    const recallSignalMatch = firstRegexMatch(RECALL_SIGNAL_PATTERNS, signalText);
...
`;
  ...
  async function analyzeExplorationRelance({
    ...
    if (SOFT_RELANCE_PATTERNS.some(p => p.test(reply))) {
      ...
      const softExpression = firstMatchedPatternExpression(SOFT_RELANCE_PATTERNS, reply);
      return {
        isRelance: true,
...
`;
  ...
  async function analyzeAttentionQuality(message = "", history = [], promptRegistry = buildDefaultPromptRegistry()) {
    const context = trimHistory(history);
    const currentText = normalizeAnalyzerText(message);
    const historyText = context.map((entry) => normalizeAnalyzerText(entry?.content || "")).join(" ");
    ...
    const engagement = currentText.length >= 80 || /[?]/.test(message) || activeSignals.some((pattern) => pattern.test(combinedText))
      ? "active"
      : "passive";

    const quality = currentText.length >= 120 || activeSignals.some((pattern) => pattern.test(combinedText))
      ? "open"
...
`;
  ...
  async function analyzeSomaticSignal(message = "", history = [], promptRegistry = buildDefaultPromptRegistry()) {
    const text = normalizeAnalyzerText(message);
    const SOMATIC_TECHNICAL_CONTEXT_PATTERNS = [
      /\b(corps|poitrine|ventre|gorge|machoire|epaules|dos|tete|nuque|souffle|respiration|coeur|serre|serrement|pression|chaleur|froid|boule|tension|palpitation|n\u0153ud|noeud)\b/
    ];
    const SOMATIC_LOCALIZATION_BLOCK_PATTERNS = [
      /\b(j'arrive pas a dire ou|je n'arrive pas a dire ou|je sais pas ou|impossible de localiser|pas reussi a localiser|ca m'enerve de pas reussir|ca m'enerve de ne pas reussir|ca me frustre de pas reussir)\b/
    ];

    const somaticSignalMatch = firstRegexMatch(SOMATIC_TECHNICAL_CONTEXT_PATTERNS, text);
    const localizationBlockedMatch = firstRegexMatch(SOMATIC_LOCALIZATION_BLOCK_PATTERNS, text);
    const somaticLocalizationBlocked = Boolean(localizationBlockedMatch);

    if (!somaticSignalMatch) {
      return {
        ...
        source: "deterministic_no_signal",
        deterministicEvidence: [
          `somatic_signal_guard_no_signal | expression: ${regexListToExpression(SOMATIC_TECHNICAL_CONTEXT_PATTERNS)} | match: none`,
          ...(somaticLocalizationBlocked
            ? [`somatic_localization_guard_active | expression: ${regexListToExpression(SOMATIC_LOCALIZATION_BLOCK_PATTERNS)} | match: "${localizationBlockedMatch}"`]
            : [])
        ]
      };
    }
    ...
    try {
      ...
      const r = await client.chat.completions.create({
        ...
        max_completion_tokens: 40,
        messages: [
          { role: "system", content: promptRegistry.ANALYZE_SOMATIC_SIGNAL || "" },
          ...context.map(m => ({ role: m.role, content: m.content })),
          { role: "user", content: message }
        ]
      });
      ...
      const raw = (r.choices?.[0]?.message?.content || "").replace(/```json|```/g, "").trim();
      const parsed = JSON.parse(raw);
      const somaticRelevant = parsed.somaticRelevant === true
        ? true
        : parsed.somaticRelevant === false
          ? false
          : Boolean(somaticSignalMatch);

      return {
        somaticSignalActive: somaticRelevant,
        somaticLocalizationBlocked,
        regexMatch: somaticSignalMatch,
        source: "llm",
        deterministicEvidence: [
          `somatic_signal_guard_active | expression: ${regexListToExpression(SOMATIC_TECHNICAL_CONTEXT_PATTERNS)} | match: "${somaticSignalMatch}"`,
          ...(somaticLocalizationBlocked
            ? [`somatic_localization_guard_active | expression: ${regexListToExpression(SOMATIC_LOCALIZATION_BLOCK_PATTERNS)} | match: "${localizationBlockedMatch}"`]
            : []),
          `somatic_signal_llm_verdict | match: "${somaticRelevant ? "true" : "false"}"`
        ]
      };
    } catch {
      return {
        ...
        deterministicEvidence: [
          ...
          ...(somaticLocalizationBlocked
            ? [`somatic_localization_guard_active | expression: ${regexListToExpression(SOMATIC_LOCALIZATION_BLOCK_PATTERNS)} | match: "${localizationBlockedMatch}"`]
            : []),
          `somatic_signal_llm_verdict | match: "false"`
        ]
      };
    }
  }
  ...
  async function analyzeUserRegister(message = "") {
    ...
    const hasFormalSignals = formalSignals.some((pattern) => pattern.test(text));
    const hasFamiliarSignals = familiarSignals.some((pattern) => pattern.test(text));

    const userRegister = hasFormalSignals && !hasFamiliarSignals
      ? "soutenu"
  ...
  async function analyzeMemoryUpdateNeeds({
    ...
    const SKIP_STATES = ["info_pure", "info_features", "info_psychoeducation", "closure", "discharge_dysregulated", "n2_crisis"];
    ...
    const prevBase = previousConversationState
      ? (previousConversationState.startsWith("exploration") ? "exploration" : previousConversationState.split("_")[0])
      : null;
    const currBase = conversationState.startsWith("exploration") ? "exploration" : conversationState.split("_")[0];
    ...
    // LLM decision for ambiguous cases (exploration, discharge_regulated, stabilization, alliance_rupture)
    try {
      const r = await client.chat.completions.create({
        model: MODEL_IDS.analysis,
        temperature: 0,
        ...
        messages: [
          {
            role: "system",
            content: promptRegistry.ANALYZE_MEMORY_UPDATE_NEEDS || "Tu decides si la memoire conversationnelle doit etre mise a jour ce tour. Reponds uniquement : {\"shouldUpdate\": true|false, \"reason\": \"default\"|\"escalation\"|\"significant_shift\"}"
          },
          {
            role: "user",
      ...
      return {
        ...
        reason: ["default", "escalation", "significant_shift"].includes(parsed.reason) ? parsed.reason : "default",
        confidence: 0.9,
        source: "llm"
      };
    } catch {
