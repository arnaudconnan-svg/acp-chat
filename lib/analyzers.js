...
`;
    ...
    try {
      const r = await client.chat.completions.create({
        ...
        max_tokens: 80,
        messages: [
          { role: "system", content: promptRegistry.ANALYZE_ATTENTION_QUALITY },
          { role: "user", content: user }
        ]
      });
...
`;
  ...
  async function analyzeInterpretationRejection({
    ...
    const REJECTION_CANDIDATE_PATTERNS = [
      ...
      /\btu racontes n'importe quoi\b/i,
      /\ba cote de la plaque\b/i,
      /\bconseils? a la con\b/i,
      /\bpas vraiment\b/i,
...
`;
  ...
  async function analyzeSomaticSignal(message = "") {
    ...
    const somaticSignalMatch = firstRegexMatch(SOMATIC_SIGNAL_PATTERNS, text);
    const localizationBlockedMatch = firstRegexMatch(SOMATIC_LOCALIZATION_BLOCK_PATTERNS, text);
    const somaticSignalActive = Boolean(somaticSignalMatch);
    const somaticLocalizationBlocked = Boolean(localizationBlockedMatch);

    const deterministicEvidence = [];
    if (somaticSignalMatch) {
      deterministicEvidence.push(
        `somatic_signal_guard_active | expression: ${regexListToExpression(SOMATIC_SIGNAL_PATTERNS)} | match: "${somaticSignalMatch}"`
      );
    }
    if (localizationBlockedMatch) {
      deterministicEvidence.push(
