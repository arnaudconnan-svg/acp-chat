...
`;
  ...
  async function analyzeSuicideRisk(
    ...
    try {
      ...
      const idiomaticDeathExpression =
        obj.idiomaticDeathExpression === true ||
        hasIdiomaticDeathExpressionMarker(message);
      ...
      if (suicideLevel === 'N0' && explicitPersonalMarker) {
        suicideLevel = 'N1';
      }
  ...
  async function analyzeImminentMajorHarmRisk(
    ...
    try {
      ...
      const harmRiskLevel = ['H0', 'H1', 'H2'].includes(parsed.harmRiskLevel)
        ? parsed.harmRiskLevel
        : 'H0';
      const imminenceBand = [
        'none',
        'immediate',
      ...
      ].includes(parsed.imminenceBand)
        ? parsed.imminenceBand
        : 'none';
  ...
  // ─── Analyzer: Memory update decision (C2) ────────────────────────────────
  // Minimaliste: decide if memory should be updated this turn.
  // Skipped in specific states (info, closure, discharge_dysregulated).
  // Returns: { shouldUpdate, reason, confidence, source }
  async function analyzeMemoryUpdateNeeds({
    message = '',
    memory = '',
    ...
    // Deterministic skip: these states do not trigger memory analysis
    const SKIP_STATES = [
      'info_pure',
      'info_features',
      'info_psychoeducation',
    ...
    if (SKIP_STATES.includes(conversationState)) {
      return {
        shouldUpdate: false,
        reason: 'state_freeze',
        confidence: 1.0,
    ...
    // Forced update on interpretation rejection
    if (interpretationRejection === true) {
      return {
        shouldUpdate: true,
        reason: 'interpretation_rejected',
        confidence: 1.0,
        source: 'deterministic_force'
      };
    }

    // Forced update on state transition (from non-exploration to exploration or vice versa)
    const prevBase = previousConversationState
      ? previousConversationState.startsWith('exploration')
        ? 'exploration'
        : previousConversationState.split('_')[0]
      : null;
    const currBase = conversationState.startsWith('exploration')
      ? 'exploration'
      : conversationState.split('_')[0];
    if (prevBase && prevBase !== currBase) {
      return {
        shouldUpdate: true,
    ...
    // LLM decision for ambiguous cases (exploration, discharge_regulated, alliance_rupture)
    try {
      const r = await client.chat.completions.create({
        model: MODEL_IDS.analysis,
        temperature: 0,
        ...
        messages: [
          {
            role: 'system',
            content:
              promptRegistry.ANALYZE_MEMORY_UPDATE_NEEDS ||
              'Tu decides si la memoire conversationnelle doit etre mise a jour ce tour. Reponds uniquement : {"shouldUpdate": true|false, "reason": "default"|"escalation"|"significant_shift"}'
          },
          {
            role: 'user',
            content: `Message actuel:\n${message}\n\nEtat: ${conversationState}\n\nMémoire actuelle:\n${String(memory || '').slice(0, 300)}`
          }
        ]
      });
      ...
      return {
        ...
        reason: ['default', 'escalation', 'significant_shift'].includes(
          parsed.reason
        )
          ? parsed.reason
          : 'default',
      ...
      // On LLM error, default to periodic update
      return {
        shouldUpdate: true,
        reason: 'default_periodic',
        confidence: 0.5,
        source: 'llm_fallback'
      };
    }
