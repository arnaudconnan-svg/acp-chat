...
`;
  ...
  async function analyzeUserRegister(message = '') {
    ...
    const userRegister =
      hasFormalSignals && !hasFamiliarSignals
        ? 'soutenu'
        : hasFamiliarSignals && !hasFormalSignals
          ? 'familier'
          : 'courant';

    return {
      userRegister,
      formalAddress: hasFormalSignals && !hasFamiliarSignals,
      source: 'deterministic_user_register',
      deterministicEvidence: [
        `user_register_guard | expression: formal=${hasFormalSignals}, familiar=${hasFamiliarSignals} | match: ${userRegister}`
      ]
    };
  }
