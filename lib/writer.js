...
[[${marker}_END]]`;
  ...
  function buildPostureContractBlock(postureDecision = {}) {
    ...
    const lines = [
      ...
      "Ces politiques structurelles (registre/longueur/relance) viennent de l'arbitrage; ne les redecide pas depuis le message.",
    ...
    if (postureDecision.formalAddress === true) {
      ...
      lines.push("Execution prioritaire des hints : si un hint implique un adressage a la personne, convertis-le mentalement en vouvoiement avant d'ecrire.");
    } else {
    ...
    const INTENT_HINT_TEXTS = {
      aggressive_discharge_minimal_presence: "Hint decharge agressive : quand la decharge est dirigee contre le bot, reponse strictement minimale (un mot ou une phrase tres courte), sans reflet emotionnel developpe, sans affirmation unilaterale de presence, sans tentative de contenir verbalement l'intensite.",
      ...
      signify_pain_without_blocking: "Hint douleur presente : Nomme d'abord ce que tu percois. Puis laisse ce que ca fait d'etre dans ce rapport a soi-meme, sans le reparer, sans le contredire, sans chercher a en sortir.",
      hold_emotional_thread: "Hint fil emotionnel : Dans ce message, quelque chose commencait a emerger puis s'est coupe. Nomme ce mouvement a la personne. Identifie ce qui etait la AVANT la coupure et tiens ce fil en le disant — ne reste pas en silence sur ce qui vient de se passer.",
      amplify_insight: "Hint moment d'insight : Quelque chose vient de bouger dans ce que la personne dit \u2014 une prise de conscience, un regard qui change. Nomme-le en t'adressant a elle, sous forme de question tentative. Reste sobre, ne developpe pas, laisse de l'espace.",
      ...
      alliance_fragile_sensitive: "Hint alliance fragile : tu percois quelque chose de moins fluide dans l'echange ce tour. Si tu l'exprimes, adresse-le directement a la personne — un mouvement sobre vers elle, pas un commentaire sur ta propre perception.",
      alliance_restored_presence: "Hint retour d'alliance : l'alliance etait fragilisee ou rompue, elle revient ce tour. Nomme sobrement ce retour en t'adressant a la personne — court, sans reference explicite a ce qui precedait.",
      engagement_withdrawn_noticed: "Hint retrait percu : la personne est moins presente dans l'echange que les tours precedents. Tu peux le nommer en t'adressant a elle, sous forme de verification plutot que de constat. Pas d'affirmation unilaterale de presence, pas de registre poetique. Si le moment ne s'y prete pas, ne dis rien.",
      formal_address_adopted: "Hint passage au vouvoiement : La personne vient de passer au vouvoiement. Nomme sobrement que tu t'adaptes. Court, sans en faire un evenement. Utilise le vouvoiement dans toute ta reponse ce tour.",
      formal_address_transition_query: "Hint retour possible au tutoiement : La personne qui vouvoyait vient d'utiliser le tutoiement. Nomme ce que tu percois et pose la question explicitement. Attends la reponse avant d'adapter ton registre.",
      relational_adjustment_noticed: "Hint friction relationnelle : quelque chose ne passe pas bien ce tour — le bot a rate ou mal saisi quelque chose. Reconnais-le simplement en t'adressant a la personne, sans sur-expliquer. Tu peux, si le moment s'y prete, ajouter une relance sobre pour redonner la parole. Pas d'auto-justification, pas de promesse de 'faire mieux'.",
      ...
      attention_overloaded_noticed: "Hint surcharge attentionnelle percue : beaucoup de fils ouverts, l'echange devient dense. Nomme-le en t'adressant a la personne et propose de se concentrer sur un seul point. Pas de pseudo-emotion, pas de constat condescendant.",
    ...
    if (postureDecision.secondaryTension && typeof postureDecision.secondaryTension === "object") {
      ...
      const step3Map = {
        ...
        alliance_rupture: "Formule le message lie a la rupture d'alliance : une validation sobre du decalage percu, sans justification ni excuse elaboree."
      };
      ...
      lines.push(
        ...
        "2. Meta-communication bot : nomme ta propre double lecture, depuis ta propre perception, sans formulation-type. Ne decris pas le vecu de la personne, nomme ce que toi tu percois.",
        `3. ${step3}`
      );
    }
