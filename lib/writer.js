...
[[${marker}_END]]`;
  ...
  function buildPostureContractBlock(postureDecision = {}) {
    ...
    // Memory update signal instruction: ask writer to start response with JSON flag
    lines.push("");
    lines.push("Signal de mise à jour mémoire intersession:");
    lines.push("Commence ta réponse par une ligne JSON unique : {\"shouldUpdateMemory\": true} si ce tour ajoute du CONTENU GENUINELY NOUVEAU (mouvements, contexte, patterns stables) qui mérite d'être retenu en mémoire intersession. {\"shouldUpdateMemory\": false} si ce tour est une formulation variée mais couvre un terrain déjà connu ou n'apporte rien de nouveau à retenir.");
    lines.push("Ensuite, fournis ta réponse normale.");
  ...
  function parseMemoryUpdateSignal(rawResponse = "") {
    const text = String(rawResponse || "").trim();
    if (!text) return { shouldUpdateMemory: true, reply: "Je t'ecoute.", signalSource: "default" };
    ...
    try {
      ...
      if (typeof parsed.shouldUpdateMemory === "boolean") {
        const restOfReply = lines.slice(1).join("\n").trim() || "Je t'ecoute.";
        return {
          shouldUpdateMemory: parsed.shouldUpdateMemory,
          reply: restOfReply,
          signalSource: "llm_signal"
        };
      }
    } catch (e) {
      // JSON parse failed or no valid signal; treat entire response as reply
    }

    // Fallback: no signal found, default to true and treat all text as reply
    return {
      shouldUpdateMemory: true,
      reply: text,
      signalSource: "no_signal_fallback"
    };
  }
  ...
  async function generateReply({
    ...
    const { shouldUpdateMemory, reply, signalSource } = parseMemoryUpdateSignal(rawContent);
