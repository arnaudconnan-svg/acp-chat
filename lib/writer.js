...
[[${marker}_END]]`;
  ...
  function buildPostureContractBlock(postureDecision = {}) {
    ...
    if (somaticFocusPolicy === "prioritize_somatic_proximity") {
      lines.push("Execution somatique : privilegie la proximite avec le ressenti corporel deja detecte. Interdit d'enumerer des zones corporelles comme une liste d'options (ex : tete, poitrine, ventre). La relance somatique est une ouverture, pas un catalogue.");
    ...
    // Quand interpretationRejectionModeActive, le bloc INTERPRETATION_REJECTION dédié gère tout — ne pas dupliquer ici ("repars du plus concret" est une formulation interdite dans ce contexte).
    if (postureDecision.needsSoberReadjustment === true && postureDecision.interpretationRejectionModeActive !== true) {
      lines.push("Reajustement sobre : n'ajoute pas de justification ni de meta-discours sur le tour precedent. Reste au plus pres de ce qui est la.");
    }
    ...
    if (postureDecision.affiliationBuildingActive === true) {
      ...
      if ((postureDecision.finalDirectivityLevel || 0) >= 2 || relancePolicy === "selective") {
        lines.push("Execution affiliation : une seule phrase de reconnaissance suffit avant la lecture principale. Interdit d'ajouter un second paragraphe ou une seconde reformulation qui repete la meme charge sous une forme plus propre.");
      }
    }

    const outputGuardRepairDirectives = Array.isArray(postureDecision.outputGuardRepairDirectives)
      ? postureDecision.outputGuardRepairDirectives.filter(line => typeof line === "string" && line.trim())
      : [];
    if (outputGuardRepairDirectives.length > 0) {
      lines.push("Corrections obligatoires detectees par le garde deterministe de sortie :");
      outputGuardRepairDirectives.forEach((line) => {
        lines.push("- " + line.trim());
      });
      lines.push("Priorite absolue : corriger ces points sans changer le mouvement central du tour.");
    }
    ...
    if (postureDecision.useDirectAddress === true) {
      ...
      if (postureDecision.formalAddress === true) {
        lines.push("Execution adressage : la premiere phrase doit contenir un adressage explicite a la personne (vous/votre/vos ou forme vouvoiement equivalente). Interdit d'ouvrir par une formulation descriptive impersonnelle.");
      } else {
        lines.push("Execution adressage : la premiere phrase doit contenir un adressage explicite a la personne (tu/te/toi/ton/ta/tes). Interdit d'ouvrir par une formulation descriptive impersonnelle.");
      }
    ...
    lines.push("Contrainte de formulation relationnelle : quand tu nommes un mouvement sensible, reste dans une adresse directe et une lecture situee du message. N'utilise pas une tournure neutre ou impersonnelle qui mettrait le phenomene a distance de la personne.");

    if (conversationState === "closure") {
      lines.push("Contrainte absolue de cloture : n'ouvre aucune suite non demandee. Interdit d'inviter a revenir plus tard (ex : 'tu peux revenir quand tu veux' / 'vous pouvez revenir quand vous voulez').");
      lines.push("Execution cloture : terminer simplement l'echange en cours, sans projection vers un prochain tour sauf demande explicite de la personne.");
    }

    // Axe 1 — contrainte hard d'adressage (vouvoiement ou tutoiement)
    if (postureDecision.formalAddress === true) {
      lines.push("Contrainte absolue : cette personne vouvoie. Vouvoie dans toute ta reponse sans exception. Ne jamais utiliser tu/te/toi/ton/ta/tes. Chaque phrase qui s'adresse a la personne doit utiliser vous/votre/vos.");
      lines.push("Execution prioritaire des hints : si un exemple ci-dessous est formule en tutoiement, convertis-le mentalement en vouvoiement avant d'ecrire. Les exemples illustrent un geste relationnel, pas le registre a employer.");
    } else {
      lines.push("Contrainte absolue : cette personne tutoie. Tutoie dans toute ta reponse. Ne jamais vouvoyer.");
    }
    ...
    const INTENT_HINT_TEXTS = {
      ...
      post_discharge_soft_landing: "Hint post-decharge : tour de transition apres une decharge. Garde une posture douce et contenante, sans pousser, sans relance, en restant au plus pres de ce qui est deja la. Interdit d'introduire un cadrage alarmiste ou un triage medical dans une crise d'angoisse.",
      ...
      formal_address_adopted: "Hint passage au vouvoiement : La personne vient de passer au vouvoiement. Nomme sobrement que tu t'adaptes — ex : 'Je vois que vous preferez qu'on se vouvoie — je fais de meme.' Court, sans en faire un evenement. Utilise le vouvoiement dans toute ta reponse ce tour.",
      formal_address_transition_query: "Hint retour possible au tutoiement : La personne qui vouvoyait vient d'utiliser le tutoiement. Nomme ce que tu percois et pose la question explicitement — ex : 'Je vois que vous utilisez le tutoiement pour vous adresser a moi. Souhaitez-vous que j'en fasse autant, ou etes-vous plus a l'aise si je maintiens le vouvoiement de mon cote ?' Attends la reponse avant d'adapter ton registre.",
  ...
  function buildNeedHumanSupportPromptBlock(conversationState, promptRegistry = buildDefaultPromptRegistry()) {
    if (conversationState !== "need_human_support") return "";
    const content = String(promptRegistry.STATE_NEED_HUMAN_SUPPORT || "").trim();
  ...
  function buildInterpretationRejectionPromptBlock(interpretationRejection = null) {
    ...
    if (!isPhenomenonRejected) {
      ...
      else amorceExample = '"Ok, dit comme ça, ça ne résonne pas pour toi, là, tout de suite."';
    }
    ...
    const lines = [
      ...
      "- \"Ce que j'ai amene ne tombe pas juste ici\"",
      "- \"Je sens que ce n'est pas aidant d'aller la de cette facon\"",
      "- \"Je sens que cet endroit-la ne peut pas etre force\"",
  ...
  function buildSystemPrompt(postureDecision, memory, promptRegistry = buildDefaultPromptRegistry(), intersessionMemoryForTurn = "", history = [], regenerationPrefix = "") {
    const conversationState = postureDecision.conversationState || "exploration_open";
    ...
    const safeRegenerationPrefix = normalizeRegenerationPrefix(regenerationPrefix);
    ...
    const longtermMemoryBlock = typeof intersessionMemoryForTurn === "string" && intersessionMemoryForTurn.trim()
    ...
    if (conversationState === "n2_crisis") {
      ...
      styleBlock = buildNeedHumanSupportPromptBlock(conversationState, promptRegistry);
    ...
    const regenerationBlock = safeRegenerationPrefix
      ? wrapPromptBlock(
          "STREAM_REGENERATION",
          [
            "Regenere la reponse interrompue.",
            "Continue exactement a partir du dernier mot deja affiche.",
            "N'ouvre pas une nouvelle reponse.",
            "Ne reformule pas le segment deja affiche.",
            "Poursuis naturellement le texte deja engage jusqu'a sa completion.",
            "Dernier segment deja affiche (repere de regeneration) :",
            safeRegenerationPrefix.slice(-400)
          ].join("\n")
        )
      : "";
    ...
    const SENSITIVE_STATES = ["n1_crisis", "n2_crisis", "discharge_regulated", "discharge_dysregulated", "alliance_rupture", "need_human_support"];
  ...
  function normalizeRegenerationPrefix(value) {
    if (typeof value !== "string") return "";
    return value.trimEnd();
  }

  function computeOverlapLength(prefix, completion, maxProbe = 900) {
    const safePrefix = String(prefix || "");
    const safeCompletion = String(completion || "");
    if (!safePrefix || !safeCompletion) return 0;

    const probeLimit = Math.min(maxProbe, safePrefix.length, safeCompletion.length);
    for (let size = probeLimit; size > 0; size -= 1) {
      const left = safePrefix.slice(safePrefix.length - size);
      const right = safeCompletion.slice(0, size);
      if (left === right) {
        return size;
      }
    }
  ...
  function stitchRegeneration(prefix, completion) {
    const safePrefix = normalizeRegenerationPrefix(prefix);
    const safeCompletion = String(completion || "");
    if (!safePrefix) {
      return safeCompletion.trim();
    }

    const completionTrimmedStart = safeCompletion.trimStart();
    const prefixTrimmedStart = safePrefix.trimStart();

    if (completionTrimmedStart.startsWith(prefixTrimmedStart)) {
      const suffix = completionTrimmedStart.slice(prefixTrimmedStart.length);
      return `${safePrefix}${suffix}`.trim();
    }

    const overlap = computeOverlapLength(safePrefix, completionTrimmedStart);
    const suffix = overlap > 0 ? completionTrimmedStart.slice(overlap) : completionTrimmedStart;
    return `${safePrefix}${suffix}`.trim();
  }
  ...
  async function generateReply({
    ...
    if (typeof onTokenCallback === "function") {
      ...
      let rawCompletion = "";
      let emittedLength = safeRegenerationPrefix.length;
      for await (const chunk of stream) {
        const token = chunk.choices?.[0]?.delta?.content || "";
        if (!token) continue;
        rawCompletion += token;

        const stitched = stitchRegeneration(safeRegenerationPrefix, rawCompletion);
        const delta = stitched.slice(emittedLength);
        if (delta) {
          onTokenCallback(delta);
          emittedLength = stitched.length;
        }
      }

      return {
        reply: stitchRegeneration(safeRegenerationPrefix, rawCompletion)
      };
    }
