...
[[${marker}_END]]`;
  ...
  function getDischargePrompt(promptRegistry = buildDefaultPromptRegistry()) {
    const dischargeBlock = String(promptRegistry.STATE_DISCHARGE_CONTACT || "").trim();
    return wrapPromptBlock("STATE_CONTACT", dischargeBlock);
  }
  ...
  function getRelationalAdjustmentPrompt(promptRegistry = buildDefaultPromptRegistry()) {
    const adjustmentBlock = String(promptRegistry.STATE_RELATIONAL_ADJUSTMENT || "").trim();
    return wrapPromptBlock("STATE_RELATIONAL_ADJUSTMENT", adjustmentBlock);
  }
  ...
  function buildRelationalAdjustmentPromptBlock(relationalAdjustmentTriggered = false, promptRegistry = buildDefaultPromptRegistry(), relationalAdjustmentDepth = "moderate") {
    ...
    const adjustmentBlock = String(promptRegistry.STATE_RELATIONAL_ADJUSTMENT || "").trim();
    return wrapPromptBlock("STATE_RELATIONAL_ADJUSTMENT", depthInstruction + "\n\n" + adjustmentBlock);
  }
  ...
  function buildContactStatePromptBlock(conversationState = null, promptRegistry = buildDefaultPromptRegistry()) {
    if (conversationState !== "contact") return "";
    const content = String(promptRegistry.STATE_CONTACT || "").trim();
    return content ? wrapPromptBlock("STATE_CONTACT", content) : "";
  }
  ...
  function buildInterpretationRejectionPromptBlock(interpretationRejection = null) {
    ...
    const lines = [
      ...
      "- nomme brievement que ce qui a ete propose ne resonne pas ou n'aide pas pour la personne (situe sur elle, pas sur toi) — une phrase sobre suffit, pas de justification ni de defense de ta part",
      "- enchaîne immediatement sur une piste utile ou une question qui repart du plus concret",
  ...
  function buildSystemPrompt(postureDecision, memory, promptRegistry = buildDefaultPromptRegistry(), interpretationRejection = null, intersessionMemoryCompressed = "") {
    const conversationState = postureDecision.conversationState || "exploration_open";
    ...
    if (conversationState === "n2_crisis") {
      ...
      styleBlock = buildContactStatePromptBlock(conversationState, promptRegistry);
