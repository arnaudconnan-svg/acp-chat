...
[[${marker}_END]]`;
  ...
  function buildSystemPrompt(postureDecision, memory, promptRegistry = buildDefaultPromptRegistry(), intersessionMemoryForTurn = "", history = [], continuationPrefix = "") {
    const conversationState = postureDecision.conversationState || "exploration_open";
    ...
    const safeContinuationPrefix = normalizeContinuationPrefix(continuationPrefix);
    ...
    const continuationResumeBlock = safeContinuationPrefix
      ? wrapPromptBlock(
          "STREAM_RESUME",
          [
            "Reprise de reponse interrompue.",
            ...
            "Dernier segment deja affiche (repere de continuation) :",
            safeContinuationPrefix.slice(-400)
          ].join("\n")
        )
      : "";
  ...
  function normalizeContinuationPrefix(value) {
  ...
  function stitchContinuation(prefix, completion) {
    const safePrefix = normalizeContinuationPrefix(prefix);
    const safeCompletion = String(completion || "");
    if (!safePrefix) {
  ...
  async function generateReply({
    ...
    if (typeof onTokenCallback === "function") {
      ...
      let rawCompletion = "";
      let emittedLength = safeContinuationPrefix.length;
      for await (const chunk of stream) {
        const token = chunk.choices?.[0]?.delta?.content || "";
        if (!token) continue;
        rawCompletion += token;

        const stitched = stitchContinuation(safeContinuationPrefix, rawCompletion);
      ...
      return {
        reply: stitchContinuation(safeContinuationPrefix, rawCompletion)
      };
    }
