...
[[${marker}_END]]`;
  ...
  function buildPostureContractBlock(postureDecision = {}) {
    ...
    const INTENT_HINT_TEXTS = {
      ...
      procedural_temptation_light: "Hint tentation procedurale (leger) : Un contexte technique a ete detecte alors qu'on est en exploration. Nomme sobrement avec un peu d'auto-derision que tu vois la porte technique mais que tu ne la prends pas — ex : 'Je pourrais facilement vous repondre de facon tres technique, presque trop bien meme. Mais je crois qu'on raterait quelque chose d'important si je faisais ca.' ou 'Je sens la tentation de vous faire une reponse bien rangee, bien pratique. Ce serait assez confortable pour moi... et probablement un peu a cote pour vous.' Rester court, puis repartir de la personne.",
      procedural_temptation_neutral: "Hint tentation procedurale (charge elevee) : Un contexte technique a ete detecte alors qu'on est en exploration avec une charge emotionnelle elevee. Sobe, sans humour — ex : 'Je pourrais vous repondre de facon tres technique ici, mais je crois que ce serait a cote de ce qui se passe vraiment pour vous.' Court. Puis repartir de la personne.",
  ...
  function buildSystemPrompt(postureDecision, memory, promptRegistry = buildDefaultPromptRegistry(), intersessionMemoryCompressed = "", history = []) {
    const conversationState = postureDecision.conversationState || "exploration_open";
