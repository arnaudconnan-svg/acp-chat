...
function computeAffiliationFinalScore(window = []) {
  ...
  const normalizedWindow = window
    .map((value) => (typeof value === "number" ? Math.max(0, Math.min(1, value)) : 0));

  const latest = normalizedWindow[normalizedWindow.length - 1] || 0;
  const peak = Math.max(...normalizedWindow);
  const average = normalizedWindow.reduce((sum, value) => sum + value, 0) / normalizedWindow.length;
...
function computeAffiliationEstablished(window = []) {
  return computeAffiliationFinalScore(window) >= AFFILIATION_FUNCTIONAL_THRESHOLD;
}
...
function buildPostureDecision({
  ...
  const explicitClarificationNeed = /\bje n'ai pas compris ta question\b|\bj'ai pas compris ta question\b|\bje n'ai pas compris ce que tu me demandes\b|\bj'ai pas compris ce que tu me demandes\b|\bje ne comprends pas ta question\b|\bje comprends pas ta question\b/.test(normalizedMessage);
  ...
  const markWriterHintInactive = (hint, reason) => {
    ...
    const alreadyPresent = writerIntentHintsInactive.some((entry) => entry.hint === hint && entry.reason === reason);
    if (!alreadyPresent) {
  ...
  const phraseLengthPolicy = responseRegister === "familier"
    ? "courte"
    : (["alliance_rupture", "need_human_support", "closure", "discharge_dysregulated"].includes(effectiveConversationState) ? "courte" : "moyenne");
  ...
  if (contactAnalysis?.selfCriticismLevel === "high") {
    ...
    if (affiliationEstablished) {
      ...
      markWriterHintInactive("auto_compassion_door_open", "affiliation_not_established");
    }
  }
  ...
  if (emotionalDecenteringAnalysis?.emotionalDecentering === true && baseState !== "discharge") {
    if (affiliationEstablished) {
      ...
      markWriterHintInactive("hold_emotional_thread", "affiliation_not_established");
      markWriterHintInactive("auto_compassion_door_open", "affiliation_not_established");
    }
  }
  ...
  return {
    ...
    secondaryTension: (() => {
      ...
      const PRIORITY = { alliance_rupture: 4, discharge: 3, info: 2, exploration: 1 };

      const candidates = [];
      // C2 candidate (d\u00e9j\u00e0 filtr\u00e9 \u2265 medium par electActiveStateFromCandidates)
      const ALLOWED_SECONDARY_FAMILIES = new Set(["alliance_rupture", "discharge", "info", "exploration"]);
      if (secondaryTension && secondaryTension.family !== convBase && ALLOWED_SECONDARY_FAMILIES.has(secondaryTension.family)) {
