...
function buildPostureDecision({
  ...
  // E4 — signal override before state resolution
  let signalForStateResolution = detectedState;
  if (emotionalDecenteringAnalysis?.emotionalDecentering === true && detectedState === "exploration") {
    signalForStateResolution = "contact";
  }
  ...
  const intent = STATE_INTENT[effectiveConversationState] || "explorer librement";
  ...
  // Gate limiting_belief : seuil de confiance plus élevé requis pour éviter les faux positifs
  // (surcharge situationnelle, fatigue, évaluation réaliste d'une contrainte ne doivent pas déclencher)
  const limitingBeliefValidated = theoreticalOrientationSignal === "limiting_belief"
    && orientationConfidence >= 0.7;
  ...
  // Option A — ne plus forcer un changement d'etat vers contact quand l'affiliation n'est
  ...
  if (theoreticalOrientationSignal === "relational_need" && detectedState === "exploration") {
    if (!effectiveForbidden.includes("interpretive_hypothesis")) effectiveForbidden.push("interpretive_hypothesis");
  }

  if (theoreticalOrientationSignal === "limit_expression" && detectedState === "exploration") {
    if (!effectiveForbidden.includes("relance")) effectiveForbidden.push("relance");
  }

  // C3 — restrictions post-discharge
  if (conversationState === "contact" && normalizedPreviousConversationState
      && baseStateOf(normalizedPreviousConversationState) === "discharge") {
    if (!effectiveForbidden.includes("relance")) effectiveForbidden.push("relance");
    if (!effectiveForbidden.includes("interpretive_hypothesis")) effectiveForbidden.push("interpretive_hypothesis");
  ...
  if (contactAnalysis?.selfCriticismLevel === "high" && detectedState === "contact") {
  ...
  if (contactAnalysis?.meaningProtest === true && detectedState === "contact") {
    if (!effectiveForbidden.includes("relance")) effectiveForbidden.push("relance");
    if (!effectiveForbidden.includes("interpretive_hypothesis")) effectiveForbidden.push("interpretive_hypothesis");
  }

  // E3 — insightMoment en mode contact
  if (contactAnalysis?.insightMoment === true && detectedState === "contact") {
    if (!effectiveForbidden.includes("relance")) effectiveForbidden.push("relance");
  ...
  if (theoreticalOrientationSignal === "unfinished_business" && detectedState === "exploration") {
    if (!effectiveForbidden.includes("interpretive_hypothesis")) effectiveForbidden.push("interpretive_hypothesis");
  ...
  } else if (detectedState === "discharge_regulated") {
    emotionSequenceStage = "secondary";
  } else if (detectedState === "contact" && contactAnalysis?.selfCriticismLevel === "high") {
    emotionSequenceStage = "secondary";
  } else if (detectedState === "contact" && contactAnalysis?.meaningProtest === true) {
    emotionSequenceStage = "need_access";
  } else if (detectedState === "contact" && contactAnalysis?.insightMoment === true) {
    emotionSequenceStage = "primary_adaptive";
  } else if (detectedState === "exploration" && stagnationTurns >= 3) {
...
function buildDebug(
  ...
  if (detectedState === "contact") lines.push("state: CONTACT");
  ...
  if (detectedState === "discharge_regulated") {
    lines.push("dischargeState: CONTACT REGULE");
  }
  if (detectedState === "discharge_dysregulated") {
    lines.push("dischargeState: CONTACT DEREGULE");
  }
