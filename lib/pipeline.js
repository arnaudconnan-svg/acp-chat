...
function buildPostureDecision({
  ...
  const conversationStateKey = resolveConversationState({
    detectedMode,
    contactAnalysis,
    previousConversationStateKey,
    allianceState,
  ...
  const stateTransitionValid = normalizedPreviousConversationStateKey === null
    ? true
    : isValidTransition(normalizedPreviousConversationStateKey, conversationStateKey);
  ...
  const writerMode = stateToWriterMode(conversationStateKey, {
  ...
  const tensionHoldLevel = ["low", "medium", "high"].includes(interpretationRejection?.tensionHoldLevel)
    ...
    : "medium";
  const humanFieldGuardActive = situatedImpasseDetected === true
  ...
  const hasRecentRejection = (recentHistory || []).slice(-4).some(m => {
    ...
    return /c'est pas ̤a|c'est pas ca|pas vraiment|pas du tout|t'as rate|t'as rat̩|c'est faux|pas ce que je veux dire|non,? pas /.test(content);
  });
