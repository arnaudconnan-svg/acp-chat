...
function mapStoredMessageEntry(entry) {
  ...
  return {
    ...
    debugMeta:
      item.debugMeta && typeof item.debugMeta === 'object'
        ? item.debugMeta
        : null,
    stateSnapshot:
      item.stateSnapshot && typeof item.stateSnapshot === 'object'
        ? {
            memory:
              typeof item.stateSnapshot.memory === 'string'
                ? item.stateSnapshot.memory
                : '',
...
function resolveBranchSeedPayload({
  ...
  const normalizedRequestedSeed = normalizeRequestedSeedMessages(
    requestedSeedMessages
  );

  if (resolvedAnchorInput) {
    const anchorIndex = messageEntries.findIndex(
      (entry) => String(entry?.id || '') === resolvedAnchorInput
    );

    if (anchorIndex >= 0) {
      const seededMessages = messageEntries
        .slice(0, anchorIndex + 1)
        .map(mapStoredMessageEntry);
      return {
        seededMessages,
