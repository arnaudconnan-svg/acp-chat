...
const { formatHistoryForTextPrompt } = require("./llm-messages");
...
const PROCEDURAL_TONE_PATTERNS = [
...
const INSTRUMENTAL_OBJECT_PATTERNS = [
  /outil/,
  /interface/,
  /plateforme/,
  /systeme/,
...
function readFirstMatchFromPatterns(patterns, text) {
  for (const pattern of patterns) {
    const match = String(text || "").match(pattern);
    if (match && match[0]) {
      return {
        expression: formatRegex(pattern),
        value: match[0]
      };
    }
  }
...
function getProceduralInstrumentalEvidence(reply = "") {
  ...
  const proceduralToneHit = readFirstMatchFromPatterns(PROCEDURAL_TONE_PATTERNS, text);
  const instrumentalObjectHit = readFirstMatchFromPatterns(INSTRUMENTAL_OBJECT_PATTERNS, text);
  ...
  const proceduralPathTriggered = Boolean(proceduralToneHit && instrumentalObjectHit);
  const listPathTriggered = Boolean(listStructureMatch && instrumentalObjectHit);
  ...
  if (proceduralPathTriggered) {
    return {
      ...
      expression: `${proceduralToneHit.expression} + ${instrumentalObjectHit.expression}`,
      proceduralToneMatch: proceduralToneHit.value,
      instrumentalObjectMatch: instrumentalObjectHit.value,
      listStructureMatch
    };
  }
  ...
  return {
    ...
    expression: `${formatRegex(LIST_STRUCTURE_REGEX)}|${formatRegex(NUMBERED_LIST_STRUCTURE_REGEX)} + ${instrumentalObjectHit.expression}`,
    proceduralToneMatch: proceduralToneHit ? proceduralToneHit.value : null,
    instrumentalObjectMatch: instrumentalObjectHit.value,
    listStructureMatch
  };
}
...
function getTheoreticalViolationEvidence(reply = "") {
  const text = (reply || "").toLowerCase();
  const patterns = [
    ...
    // Défensivité / agentivité — stems
    "défensif", "défensive", "defensif", "defensive",
    "refoul",
    "bouclier",
    "surprot",
    // Formulations culpabilisantes observées
    "sans distance",
    "à tout prix", "a tout prix",
    "coûte que coûte", "coute que coute",
    "tenir à distance", "tenir a distance",
    // Sur-activation demandee: stems agentivite/culpabilisation
    "repli",
    "en ser",
    "raison",
    "retir",
  ...
  for (const pattern of patterns) {
    if (text.includes(pattern)) {
      return {
        reason: "theoreticalViolationRisk",
        pathway: "theoretical_heuristic_keyword",
...
function hasTheoreticalViolationHeuristic(reply = "") {
  return getTheoreticalViolationEvidence(reply) !== null;
}

// Detect attributions of intentional agency to the user (sync check).
// This targets grammatical constructions like "tu as choisi", "vous voulez",
// "tu t'en sers", and purpose clauses "pour eviter/proteger" when attributed.
function getAgencyAttributionEvidence(reply = "") {
  const text = normalizeGuardText(reply);
  const patterns = [
    /\b(?:tu|vous)\s+(?:as|avais|a|avez|es|etes|\u00eates)?\s*(?:decid|chois|voulu|veux|voulez|refus|retir|repli|serv|d\u00e9fend|defend|prot\u00e9g|proteg|\u00e9vit|evit|raisonn)\w*/,
    /\b(?:tu|vous)\s+t['\u2019]?en\s+(?:sers|servez|sers|serves|servir|es\s+servi|etais\s+servi|\u00eates\s+servi)/,
    /\b(?:tu|vous)\s+(?:te|t['\u2019]|se|s['\u2019])\s+(?:prot\u00e8g|protege|d\u00e9fend|defend|retir|repli|\u00e9vit|evit)\w*/,
    /\bpour\s+(?:te\s+|t['\u2019]|vous\s+|se\s+|s['\u2019])?(?:prot\u00e9ger|proteger|\u00e9viter|eviter|te\s+retirer|vous\s+retirer|se\s+retirer|te\s+replier|vous\s+replier|se\s+replier)/
  ];

  const hit = readFirstMatchFromPatterns(patterns, text);
  if (!hit) return null;

  return {
    reason: "agencyAttributionRisk",
    pathway: "agency_attribution_pattern",
...
function createCritic({ client, MODEL_IDS }) {
  ...
  async function applySelectiveCritic({
    ...
    const safeHistoryText = String(historyText || "").trim() || formatHistoryForTextPrompt(history);
