...
// Product contract (alpha test): monthly budget is 12 EUR with up to +25% rollover.
const MONTHLY_CAPACITY = 12;
const ROLLOVER_CAPACITY = 3;
// Reserve is modeled in usage-equivalent units (12 max), while admin display can map it to paid EUR.
const RESERVE_CAPACITY = 12;
...
function normalizeRawUsageData(rawUsageData = {}) {
  const safe =
    rawUsageData &&
    typeof rawUsageData === 'object' &&
    !Array.isArray(rawUsageData)
      ? rawUsageData
      : {};
  ...
  return {
    monthly: {
      ...
      remaining: clampNumber(
        safe.monthly && safe.monthly.remaining,
        0,
        MONTHLY_CAPACITY
      )
    },
    rollover: {
      ...
      startedWith: clampNumber(
        safe.rollover && safe.rollover.startedWith,
        clampNumber(safe.rollover && safe.rollover.remaining, 0, ROLLOVER_CAPACITY),
        ROLLOVER_CAPACITY
      ),
      remaining: clampNumber(
        safe.rollover && safe.rollover.remaining,
        0,
        ROLLOVER_CAPACITY
      )
    },
    reserve: {
      ...
      remaining: clampNumber(
        safe.reserve && safe.reserve.remaining,
        0,
        RESERVE_CAPACITY
      )
    },
...
function getEnvelopeState(rawUsageData = {}) {
  ...
  const totalAvailable =
    normalized.rollover.remaining +
    normalized.monthly.remaining +
    normalized.reserve.remaining;
...
function consumeEnvelope(rawUsageData = {}, amount = 0) {
  ...
  const nextState = {
    ...
    rollover: {
      ...
      startedWith: state.rollover.startedWith,
...
function computeGaugeSegments(amount, options = {}) {
  ...
  const segmentCount =
    Number.isInteger(options.segmentCount) && options.segmentCount > 0
  ...
  const epsilon = 1e-9;
  const hasHalfSegment = remainder + epsilon >= halfSegment;
  const isLowPositive = safeAmount > 0 && safeAmount < halfSegment;
  const topBandLight =
    fullSegments === segmentCount - 1 &&
    safeAmount < capacity - epsilon &&
    safeAmount > capacity - halfSegment + epsilon;
  ...
  for (let i = 0; i < segmentCount; i += 1) {
    ...
    if (
      i === fullSegments &&
      (hasHalfSegment || isLowPositive || topBandLight)
    ) {
...
function addComplementaryReserve(rawUsageData = {}, amount = 0) {
  ...
  const nextReserve = clampNumber(
    state.reserve.remaining + toAdd,
    0,
    RESERVE_CAPACITY
  );
