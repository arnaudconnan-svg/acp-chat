...
function normalizeDischargeState(dischargeState) {
  if (
    !dischargeState ||
    typeof dischargeState !== 'object' ||
    Array.isArray(dischargeState)
  ) {
...
function normalizeSessionFlags(flags) {
  ...
  const hasExplicitDirectivityLevel =
  ...
  const computedLevel = computeExplorationDirectivityLevel(
  ...
  return {
    ...
    crisisFollowupTurnCount:
      Number.isInteger(safe.crisisFollowupTurnCount) &&
      safe.crisisFollowupTurnCount >= 0
        ? safe.crisisFollowupTurnCount
        : 0,
    ...
    explorationCalibrationLevel: clampExplorationDirectivityLevel(
    ...
    dependencyAnalysisTurnsUntilRefresh: Number.isInteger(
      safe.dependencyAnalysisTurnsUntilRefresh
    )
      ? Math.max(0, safe.dependencyAnalysisTurnsUntilRefresh)
      : 0,
    dependencyCareTriggered: ['none', 'medium', 'high'].includes(
      safe.dependencyCareTriggered
    )
      ? safe.dependencyCareTriggered
      : 'none',
    dependencyCareMessagePending: ['medium', 'high'].includes(
      ...
      ? safe.dependencyCareMessagePending
      : false,
    dependencyCareMessagePendingTurns: Number.isInteger(
      safe.dependencyCareMessagePendingTurns
    )
      ? Math.max(0, safe.dependencyCareMessagePendingTurns)
      : 0,
    ...
    affiliationAttachmentBoostStreak: Number.isInteger(
      safe.affiliationAttachmentBoostStreak
    )
      ? Math.max(0, safe.affiliationAttachmentBoostStreak)
      : 0,
    turnsUntilIntersessionRefresh: Number.isInteger(
      safe.turnsUntilIntersessionRefresh
    )
      ? Math.max(0, safe.turnsUntilIntersessionRefresh)
      : 0,
    attentionQualityTurnsUntilRefresh: Number.isInteger(
      safe.attentionQualityTurnsUntilRefresh
    )
      ? Math.max(0, safe.attentionQualityTurnsUntilRefresh)
      : 0
  };
}
...
function registerExplorationRelance(flags, isRelance) {
  ...
  const nextWindow = [
    ...safeFlags.explorationRelanceWindow,
    isRelance === true
  ].slice(-RELANCE_WINDOW_SIZE);
...
function getExplorationStructureInstruction(
  ...
  const safeLevel = clampExplorationDirectivityLevel(
    explorationDirectivityLevel
  );
