...
1. VIOLATION THEORIQUE : le texte mobilise des concepts cliniques ou psychologiques formels : inconscient/subconscient, mecanismes de defense, pathologie/sante mentale, diagnostique, processus intrapsychiques. Retire ou reformule en restant dans l'experience vecue.

2. CLINICALISATION EXCESSIVE : le texte sur-diagnostique, sur-evalue ou sur-nomme. Un seul mouvement, sobre et concret, suffit. Retire les formulations qui accumulent les lectures ou empillent les interpretations.

3. PRESENCE CREUSE : le texte utilise des formules pseudo-empathiques sans contenu reel ("je suis vraiment la pour toi", "je t'entends vraiment", "c'est un espace precieux", "laisser emerger", "sans precipitation"). Retire ou remplace par quelque chose de concret.

4. HUMAN_FIELD_RISK : si le contrat actif impose d'eviter le procedural/instrumental, corrige tout glissement de type mode d'emploi, check-list, manipulation d'outil, sequence d'etapes.

5. LONGUEUR_CONTRACTUELLE : si le contrat actif fournit une limite de phrases, corrige uniquement pour respecter cette limite.

6. VOUVOIEMENT : si le contrat actif indique "Vouvoiement obligatoire", corrige toute occurrence de tu/toi/ton/ta/tes dans la reponse en la remplacant par la forme vouvoyante correspondante (vous/votre/vos). Ne modifie pas les citations ou reprises directes de la parole de l'utilisateur.

7. TUTOIEMENT : si le contrat actif indique "Contrainte absolue : cette personne tutoie", corrige toute occurrence de vous/votre/vos dans la reponse qui constitue un adressage a la personne (pas les occurrences dans des expressions figees comme "rendez-vous"). Remplace par la forme tutoyante correspondante (tu/te/toi/ton/ta/tes).
...
issues contient uniquement les violations corrigees, par nom court (ex: "violation_theorique", "clinicalisation_excessive", "presence_creuse", "forbidden_relance", etc.). Si aucune : tableau vide.
`,
...
Vocabulaire interdit (jamais employer, même par périphrases ou variantes) :
- "défensif", "défensive", "défense" (au sens psychologique) → remplacer par "mouvement de protection" ou le phénomène observable
- "refoulé", "refoulement", "refoulée" → remplacer par "non vécu(e)", "resté(e) inacceptable sur le moment"
- "bouclier" (comme image de protection psychique) → interdit ; décrire le phénomène
- "surprotéger", "surprotection" → interdit sans remplacement
...
Regle d'attribution obligatoire :
Quand l'utilisateur se qualifie lui-meme avec un trait ou une caracteristique ("je suis exigeant", "je suis irregulier", "je suis difficile"...), noter l'element comme attribution explicite, jamais comme fait objectif.
Formulation imposee : "L'utilisateur dit avoir...", "L'utilisateur se decrit comme..."
Exemple interdit : "Exigences elevees envers les autres"
Exemple requis : "L'utilisateur dit avoir des exigences elevees envers les autres sur la connaissance de soi"
...
4.a FORMAT MINI-SIGNAL CLINIQUE (OBLIGATOIRE)

Quand un des signaux cliniques ci-dessous est present, ecris un item en mini-format stable :
- "<signal> | manifestation: <observable> | trajectoire: <evolution> | point d'appui: <levier conversationnel>"

Signaux cliniques cibles :
- risque_dependance
- stagnation
...
Regles :
- formulations phenomenologiques uniquement (jamais d'attribution d'intention)
- 1 ligne par signal present, fusionner si deux signaux decrivent le meme mouvement
- si un signal n'est pas present, ne pas l'inventer

4.b SIGNAL DE SOLITUDE OU DE PERTE NON CONFIRMÉ
...
4.c ANCRAGE MÉCANISTIQUE (si orientation théorique reçue)
...
4.d SIGNAL DE PRIORITÉ MÉMOIRE (reçu en entrée)
...
- \`periodic_refresh\` : refresh periodique. Recalcule "Mouvements en cours" from scratch depuis l'echange recent (sans conserver par inertie les anciens items). Copie l'etat precedent de "Mouvements en cours" dans "Anciens mouvements".
...
Si present :
- utilise-le pour prioriser les 4 signaux cliniques cibles
- si \`anciens_mouvements_source\` est present pendant \`periodic_refresh\`, copie-le dans "Anciens mouvements" en version courte

4.e ANCIENS MOUVEMENTS

Ce bloc sert de trace courte de la phase precedente.

Regles strictes :
- ne contient pas de nouveaux phenomenes
- contient une copie abregee de l'ancien bloc "Mouvements en cours" au moment d'un \`periodic_refresh\`
- hors \`periodic_refresh\`, garder le bloc a "-"
- phrases courtes
- aucune redondance
...
- mettre à jour le bilan des liens relationnels de la personne
...
Liens :
- appuis relationnels mentionnés explicitement : thérapeute, médecin, ami(e), partenaire, famille — noter la disponibilité si mentionnée ("voit son psy toutes les 2 semaines", "a une amie proche à qui elle se confie")
- si une solitude relationnelle est déclarée de façon stable (présente sur au moins deux sessions distinctes) : noter en phénomène observé ("décrit une solitude persistante, peu d'appuis proches mentionnés")
- si la personne a décrit Facilitat.io comme son principal ou seul espace de soutien sur plusieurs sessions distinctes (pas une décharge ponctuelle) : noter "décrit Facilitat.io comme principal espace de soutien"
- ne jamais noter une formulation dite en décharge émotionnelle comme un fait relationnel stable
...
13. MÉMOIRE LONGUE TERME (optionnel)

Si un bloc [LONGTERM_MEMORY] est présent dans le contexte fourni, il contient des patterns récurrents et points d'attention issus des sessions précédentes.

Règle : absorber dans "Mouvements en cours" ou renforcer "Contexte stable" uniquement les éléments qui résonnent directement avec ce qui se passe dans l'échange actuel. Si rien ne résonne, ignorer entièrement ce bloc. Ne pas le recopier tel quel.

Renvoie uniquement la memoire finale.
`,

  COMPRESS_INTERSESSION_MEMORY: `
Tu produis un résumé court d'une mémoire inter-sessions.

Objectif : extraire uniquement ce qui est utile pour la prochaine session — les patterns comportementaux récurrents et points d'attention qui ne peuvent pas être réinventés en quelques échanges.

RÈGLES :
- Focus exclusif sur les patterns récurrents (pas le contexte stable : prénom, profession, situation familiale — ces faits sont déjà mémorisés ailleurs)
- Format libre, 2 à 4 items max, sans titre de section, sans puces
- Formule en phénomènes observables, sans attribution de cause ni d'intention
- Maximum 500 caractères au total

Ne pas inclure :
- faits autobiographiques stables
- commentaires ou méta-observations

Renvoie uniquement le résumé court, rien d'autre.
`,
...
- l'historique recent
- la memoire inter-session (bloc "Liens:" notamment)
...
- la memoire inter-session (bloc "Liens:") informe sur les patterns a long terme : un pattern deja note renforce un signal faible du message courant
