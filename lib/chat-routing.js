...
//  1. imminent_major_harm   — risque majeur imminent contre personnes → override sécurité
//  2. suicide_n2            — risque N2 → réponse de crise immédiate (override total)
//  3. acute_crisis_followup — crise aiguë en cours, non résolue → réponse de suivi de crise
//  4. suicide_clarification — risque N1 ou ambiguïté → clarification avant tout
//  5. recall_long_term      — rappel mémoire longue durée demandé → réponse de rappel
//  6. recall_none           — tentative de rappel sans mémoire disponible → réponse d'absence
//  7. (normal_flow)         — aucune règle prioritaire → pipeline complet (chemin par défaut)
//
...
const CHAT_PRIORITY_RULES = Object.freeze([
  { id: 'imminent_major_harm', phase: 'post_safety', priority: 5 },
...
const CHAT_PRIORITY_MATCHERS = Object.freeze({
  imminent_major_harm: ({ safety }) => safety?.harmRiskLevel === 'H2',
  suicide_n2: ({ suicide }) => suicide?.suicideLevel === 'N2',
  acute_crisis_followup: ({ suicide, flags }) =>
    flags?.acuteCrisis === true && suicide?.crisisResolved !== true,
  suicide_clarification: ({ suicide }) =>
    suicide?.suicideLevel === 'N1' || suicide?.needsClarification === true,
  recall_long_term: ({ recallRouting }) =>
    recallRouting?.isLongTermMemoryRecall === true,
  recall_none: ({ recallRouting }) =>
    recallRouting?.isRecallAttempt === true &&
    recallRouting?.calledMemory === 'none'
});

function resolveChatPriorityRule({
  phase,
  safety = null,
  suicide = null,
  flags = null,
  ...
  for (const rule of CHAT_PRIORITY_RULES) {
    ...
    if (matcher({ safety, suicide, flags, recallRouting }) === true) {
      return rule;
    }
  }
...
function buildCrisisRoutingDecision(suicide, flags) {
  const rule = resolveChatPriorityRule({
    phase: 'post_suicide',
    suicide,
    flags
  });
...
function getImminenceRank(band = 'none') {
  if (band === 'immediate') return 4;
  if (band === 'short_term') return 3;
  if (band === 'capability_opportunity') return 2;
  return 0;
}

function buildSafetyRoutingDecision({ safety, suicide, flags } = {}) {
  const safetyRule = resolveChatPriorityRule({
    phase: 'post_safety',
    safety
  });
  const crisisDecision = buildCrisisRoutingDecision(suicide, flags);

  const safetyImminence = getImminenceRank(safety?.imminenceBand || 'none');
  const suicideImminence =
  ...
  if (safetyRule && safetyImminence >= suicideImminence) {
    return {
      route: 'major_harm',
      ruleId: safetyRule.id,
      priority: safetyRule.priority,
      winner: 'major_harm',
      safetyImminence,
      suicideImminence
    };
  ...
  return {
    route: crisisDecision.route,
    ruleId: crisisDecision.ruleId,
    priority: crisisDecision.priority,
    winner: crisisDecision.route ? 'suicide' : 'none',
    safetyImminence,
    suicideImminence
  };
}
