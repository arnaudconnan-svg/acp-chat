...
function buildDirectivityText({
  ...
  return [
    safeRetainedLevel !== null
      ...
      : null,
    safeInputLevel !== null
      ? `Niveau de directivite herite (N-1) : ${safeInputLevel}/4`
      : null,
    safeUsedLevel !== null
      ? `Niveau de directivite utilise (N) : ${safeUsedLevel}/4`
      : null,
    `Fenetre de relance preparee (N+1) : [${safeWindow.map((v) => (v ? '1' : '0')).join('-')}]`,
    `Niveau de directivite prepare (N+1) : ${safeNextLevel}/4`,
    relanceStatusLabel
      ? `Statut relance asynchrone : ${relanceStatusLabel}`
      : null,
    Number.isInteger(relanceAppliedAtTurnEntrySourceTurn)
      ? `Relance asynchrone appliquee depuis le tour : ${relanceAppliedAtTurnEntrySourceTurn}`
      : null,
    typeof relanceAppliedAtTurnEntryStatus === 'string' &&
    relanceAppliedAtTurnEntryStatus.trim()
      ? `Mode d'application : ${relanceAppliedAtTurnEntryStatus.trim()}`
      : null,
    Number.isInteger(relanceAsyncTargetTurn)
      ? `Tour cible de la relance asynchrone : ${relanceAsyncTargetTurn}`
      : null
  ]
    .filter(Boolean)
    .join('\n');
...
function buildResponseDebugMeta({
  ...
  return {
    ...
    phraseLengthPolicy:
      typeof phraseLengthPolicy === 'string' ? phraseLengthPolicy : 'moyenne',
    relancePolicy:
