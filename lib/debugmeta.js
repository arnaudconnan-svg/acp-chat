...
function detectMemoryReactivationInState(memoryState = null) {
  ...
  const onGoing = Array.isArray(memoryState.onGoingMovements)
    ? memoryState.onGoingMovements
      .map(item => (item && typeof item === "object" ? String((item.text || "")).trim() : null))
      .filter(Boolean)
    : [];
...
function buildResponseDebugMeta({
  ...
  return {
    ...
    // Lot 8 fields
    affiliationScore: typeof affiliationScore === "number" ? Math.round(affiliationScore * 100) / 100 : null,
    affiliationFinalScore: typeof affiliationFinalScore === "number" ? Math.round(affiliationFinalScore * 100) / 100 : null,
