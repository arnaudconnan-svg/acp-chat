      ...
      conversationState === 'info_psychoeducation'
        ...
        : conversationState === 'info_features'
          ? Array.isArray(infoContextFlags) &&
              infoContextFlags.includes('service_contact_request')
            ? "RECOURS HUMAIN : contact avec l'équipe"
            : Array.isArray(infoContextFlags) &&
                infoContextFlags.includes('human_support_request')
              ? 'RELAIS HUMAIN : accompagnement'
              : Array.isArray(infoContextFlags) &&
...
function buildResponseDebugMeta({
  ...
  infoContextFlags = [],
  allianceAssessmentReason = null,
  allianceAssessmentSource = null,
  humanSupportProposal = 'not_indicated',
  humanSupportProposalReason = null,
  ...
  return {
    ...
    suicideLevel: ['N0', 'N1', 'N2'].includes(suicideLevel)
      ? suicideLevel
      : 'N0',
    ...
    analyzerDeterministicEvidence: Array.isArray(analyzerDeterministicEvidence)
      ...
      : [],
    memoryUpdateDecision: ['update', 'hold'].includes(memoryUpdateDecision)
      ? memoryUpdateDecision
      : 'unknown',
    memoryUpdateReason:
      typeof memoryUpdateReason === 'string' ? memoryUpdateReason : null,
    memoryUpdateSource:
      typeof memoryUpdateSource === 'string' ? memoryUpdateSource : null,
    ...
    memoryUpdateResultSource:
      typeof memoryUpdateResultSource === 'string'
        ? memoryUpdateResultSource
        : null,
    // Posture contract (V3)
    ...
    allianceAssessmentReason:
      typeof allianceAssessmentReason === 'string'
        ? allianceAssessmentReason
        : null,
    allianceAssessmentSource:
      typeof allianceAssessmentSource === 'string'
        ? allianceAssessmentSource
        : null,
    humanSupportProposal: [
      ...
      'not_indicated',
      'already_addressed'
    ].includes(humanSupportProposal)
      ? humanSupportProposal
      : 'not_indicated',
    humanSupportProposalReason:
      typeof humanSupportProposalReason === 'string'
        ? humanSupportProposalReason
        : null,
    humanSupportProposalEffective: humanSupportProposalEffective === true,
    // Lot 8 fields
    affiliationScore:
    ...
    lowContextOpeningGuardActive: lowContextOpeningGuardActive === true,
    offTopicInfoPolicy:
