...
// 9 base states — used by the transition graph.
...
// ─── baseStateOf ──────────────────────────────────────────────────────────────
// Extracts the base state from an extended conversationState.
...
function baseStateOf(state) {
  ...
  return state; // stabilization, alliance_rupture, need_human_support, closure, n1_crisis, n2_crisis
}
...
const STATE_TRANSITIONS = {
  ...
  exploration:     ["exploration", "discharge", "info", "stabilization", "alliance_rupture", "need_human_support", "closure"],
  discharge:       ["discharge", "exploration", "info", "stabilization", "alliance_rupture", "need_human_support", "closure"],
  info:            ["exploration", "discharge", "info", "stabilization", "alliance_rupture", "need_human_support", "closure"],
  stabilization:   ["stabilization", "exploration", "discharge", "info", "alliance_rupture", "need_human_support", "closure"],
  alliance_rupture:["exploration", "discharge", "info", "stabilization", "alliance_rupture", "need_human_support", "closure"],
  need_human_support:["need_human_support", "exploration", "discharge", "info", "stabilization", "closure"],
  closure:         ["exploration", "discharge", "info", "need_human_support"],
...
const STATE_FORBIDDEN = {
  ...
  need_human_support:    ["interpretive_hypothesis", "self_justification", "recap", "list"],
  closure:               ["relance", "open_question"],
  discharge_regulated:   ["interpretive_hypothesis", "open_question", "relance"],
...
const STATE_ALLOWED = {
  ...
  need_human_support:    ["contain", "simple_presence", "short_repair", "open_question"],
...
const STATE_INTENT = {
  ...
  need_human_support:    "tenir le point et ouvrir doucement vers un appui humain",
...
const STATE_CONSTRAINTS = {
  ...
  need_human_support:     { maxSentences: 4, toneConstraint: "sober" },
...
// Priority order: discharge > support need > post-discharge cooldown > info > exploration,
...
function resolveConversationState({
  ...
  // Priority 2: support need overrides exploration, info, and post-discharge cooldown.
  } else if (dependencyCareMessagePending) {
    base = "need_human_support";

  // Priority 3: post-discharge cooldown (previous turn was discharge, current is not info)
  ...
  if (base === "exploration") {
    if (allianceSignal === "rupture") {
      base = "alliance_rupture";
    } else if (
      (attentionWindow === "overloaded" && engagementLevel === "withdrawn") ||
      (attentionWindow === "overloaded" && stagnationTurns >= 2) ||
  ...
  if (closureIntent === true && base !== "discharge_regulated" && base !== "discharge_dysregulated"
      && base !== "alliance_rupture" && base !== "need_human_support") {
    base = "closure";
  }
