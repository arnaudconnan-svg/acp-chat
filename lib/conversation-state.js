...
// Extended 11-state conversationState is returned by resolveConversationState.
...
function baseStateOf(state) {
  ...
  return state; // contact, stabilization, alliance_rupture, closure, n1_crisis, n2_crisis
}
...
const STATE_TRANSITIONS = {
  ...
  n1_crisis:       ["n1_crisis", "n2_crisis", "exploration", "contact", "closure"],
  n2_crisis:       ["n2_crisis", "n1_crisis", "exploration", "contact", "closure"]
};
...
const STATE_CONSTRAINTS = {
  ...
  contact:                { maxSentences: 3, toneConstraint: "minimal" },
...
// Priority order: discharge > contact > post-discharge cooldown > info > exploration,
// then Phase B overrides (stabilization, alliance_rupture, closure).
// Returns one of the 11 extended state values.
function resolveConversationState({
  ...
  // Priority 2: soft contact
  } else if (detectedState === "contact") {
    base = "contact";
  ...
  // Phase B structural overrides apply only to exploration (not contact, discharge, or info)
  if (base === "exploration") {
  ...
  if (closureIntent === true && base !== "discharge_regulated" && base !== "discharge_dysregulated"
      && base !== "alliance_rupture" && base !== "contact") {
    base = "closure";
  }
