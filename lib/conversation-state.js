...
const WRITER_MODE_CONSTRAINTS = {
  exploration_open:      { maxSentences: 5 },
...
function resolveConversationState({
  ...
  // Priority 3: post-discharge cooldown (previous turn was discharge, now back to exploration)
  } else if (previousConversationStateKey === "discharge" && detectedMode === "exploration") {
    state = "contact";
  ...
  if (state === "exploration" || state === "contact") {
  ...
  if (closureIntent === true && state !== "discharge" && state !== "alliance_rupture") {
    state = "closure";
  }
