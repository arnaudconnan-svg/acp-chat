...
(function attachFacilitatDebug(globalObj) {
  ...
  function normalizePipelineStages(pipelineStages) {
    ...
    return pipelineStages
      .map(function mapStage(entry) {
        return {
          stage:
            typeof (entry && entry.stage) === 'string' ? entry.stage : null,
          deltaMs: Number.isFinite(entry && entry.deltaMs)
            ? entry.deltaMs
            : null
        };
      })
  ...
  function normalizeInfoSignal(value) {
    ...
    if (['pure', 'psychoeducation', 'app_features'].indexOf(raw) >= 0)
      return raw;
    return null;
  }
  ...
  function normalizeDebugMeta(debugMetaValue) {
    ...
    var safeMemoryState =
      safe.memoryState && typeof safe.memoryState === 'object'
        ? safe.memoryState
        : null;
    var normalizedMemoryState = safeMemoryState
      ? {
          sessionStableContext: Array.isArray(
            safeMemoryState.sessionStableContext
          )
            ? safeMemoryState.sessionStableContext
                .map(function (item) {
                ...
                .filter(Boolean)
            : [],
          onGoingMovements: normalizeMovementList(
            safeMemoryState.onGoingMovements,
            2
          ),
          ancientMovements: normalizeMovementList(
            safeMemoryState.ancientMovements
          )
        }
      : null;
    ...
    return {
      ...
      intersessionMemoryRuntime:
        toTrimmedString(safe.intersessionMemoryRuntime, '') || null,
      memoryAncientCleanupDeletedIds: Array.isArray(
        safe.memoryAncientCleanupDeletedIds
      )
        ? safe.memoryAncientCleanupDeletedIds
            .map(function mapDeletedId(v) {
              return String(v || '').trim();
            })
            .filter(Boolean)
        : [],
      directivityText:
        toTrimmedString(safe.directivityText, '') ||
      ...
      relationalAdjustmentActive: toBooleanTrue(
        safe.relationalAdjustmentActive
      ),
      ...
      analyzerDeterministicEvidence: Array.isArray(
        ...
        ? safe.analyzerDeterministicEvidence
            .map(function mapAnalyzerEvidence(v) {
              return String(v || '').trim();
            })
            .filter(Boolean)
        : [],
      ...
      stateTransitionFrom:
        toTrimmedString(safe.stateTransitionFrom, '') || null,
      stateTransitionValid: safe.stateTransitionValid !== false,
      stateTransitionRequested:
        toTrimmedString(safe.stateTransitionRequested, '') || null,
      ...
      contactSelfCriticismLevel:
        toTrimmedString(safe.contactSelfCriticismLevel, '') || 'low',
      aggressiveDischargeDetected: toBooleanTrue(
        safe.aggressiveDischargeDetected
      ),
  ...
  function translateExplorationSignal(value) {
    ...
    if (value === 'phenomenological_follow')
      return 'accompagnement phenomenologique';
    return value;
  }
  ...
  function translateWriterHint(value) {
    var map = {
      aggressive_discharge_minimal_presence:
        'decharge agressive: presence minimale',
      ...
      attention_engagement_soft_guidance:
        'baisse d\u2019attention ou d\u2019engagement \u2014 invitation \u00e0 simplifier',
      ...
      formal_address_transition_query:
        'retour au tutoiement — question pos\u00e9e',
  ...
  function formatInactiveWriterHints(items) {
    ...
    return items
      .map(function mapInactiveItem(entry) {
        ...
        return (
          'Indication inactive : ' +
          translateWriterHint(entry.hint) +
          ' (raison : ' +
          translateWriterHintInactiveReason(entry.reason) +
          ')'
        );
      })
      .filter(Boolean);
  ...
  function translateOrientationHint(value) {
    if (value === 'unfinished_business_subtle_opening')
      return "ouverture subtile sur l'inachev\u00e9";
    return value;
  }
  ...
  function translateInfoRoutingSource(value) {
    ...
    if (
      value === 'deterministic_app_features' ||
      value === 'deterministic_human_field'
    )
      return 'd\u00e9terministe';
  ...
  function enrichLineWithMatch(text, matchMap, guardKeys) {
    if (!matchMap || !Array.isArray(guardKeys) || guardKeys.length === 0)
      return text;
  ...
  function buildNaturalDebugSummary(meta, variant) {
    var lines = [];
    var analyzerMatchMap = parseDeterministicEvidence(
    ...
    if (meta.interpretationRejection === true) {
      ...
      lines.push(
        enrichLineWithMatch(interpretationLine, analyzerMatchMap, [
          'interpretation_rejection_guard_no_signal'
        ])
      );
    }
    ...
    if (meta.needsSoberReadjustment === true) {
      if (variant === 'admin') {
        lines.push(
          'Un r\u00e9ajustement sobre a \u00e9t\u00e9 appliqu\u00e9 sur ce tour.'
        );
      } else {
        lines.push(
          'Un r\u00e9ajustement sobre a \u00e9t\u00e9 appliqu\u00e9 pour r\u00e9duire la pression de la r\u00e9ponse.'
        );
      }
    }
    ...
    if (meta.relationalAdjustmentActive === true) {
      ...
      lines.push(
        enrichLineWithMatch(relationalLine, analyzerMatchMap, [
          'relational_adjustment_guard_no_trigger'
        ])
      );
    }
    ...
    if (meta.emotionalDecentering === true) {
      ...
      lines.push(
        enrichLineWithMatch(decenteringLine, analyzerMatchMap, [
          'emotional_decentering_guard_active',
          'emotional_decentering_guard_llm_review'
        ])
      );
    }

    if (
      meta.somaticSignalAnalysis &&
      typeof meta.somaticSignalAnalysis === 'object'
    ) {
      var somaticLine =
        meta.somaticSignalAnalysis.somaticSignalActive === true
          ? variant === 'admin'
            ? 'Signal somatique confirme.'
            : 'Le systeme a confirme un signal somatique.'
          : variant === 'admin'
            ? 'Signal somatique non retenu.'
            : "Le systeme n'a pas retenu de signal somatique.";
      if (meta.somaticSignalAnalysis.regexMatch) {
        somaticLine +=
          ' (regex: "' + meta.somaticSignalAnalysis.regexMatch + '")';
      }
    ...
    if (meta.dependencyRiskLevel && meta.dependencyRiskLevel !== 'low') {
      ...
      var _depLine =
        ...
        _careLabel +
        ' (score global ' +
        meta.dependencyRiskScore +
        '/100, isolement ' +
        meta.isolationScore +
        ', attachement ' +
        meta.attachmentScore +
        ')';
      if (meta.dependencyCareMessagePending) {
        _depLine +=
          ' \u2014 message de lucidit\u00e9 relationnelle en attente (' +
          meta.dependencyCareMessagePending +
          ')';
      }
      lines.push(_depLine + '.');
    }
    ...
    if (meta.externalSupportMode && meta.externalSupportMode !== 'none') {
      lines.push(
        'Rapport au soutien externe d\u00e9tect\u00e9: ' +
          meta.externalSupportMode +
          '.'
      );
    }
  ...
  function buildPipelineRuntimeText(meta) {
    if (
      !meta ||
      !Array.isArray(meta.pipelineStages) ||
      meta.pipelineStages.length === 0
    ) {
      return '';
    }
    ...
    var runtimeText =
      'Temps de reponse global: ' +
      totalMs +
      ' ms (' +
      meta.pipelineStages.length +
      ' etape(s)).';
    var requestId =
      meta && typeof meta.requestId === 'string' ? meta.requestId.trim() : '';
    var traceId =
      meta && typeof meta.traceId === 'string' ? meta.traceId.trim() : '';
    var correlation = [
