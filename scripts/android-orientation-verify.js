...
const SOURCE_CHECKS = [
  ...
  },
  {
    ...
    test: (content) =>
      /<activity android:name="LauncherActivity"[\s\S]*?android:screenOrientation="portrait"/m.test(
        content
      ),
...
const GENERATED_CHECKS = [
  {
    ...
    test: (content) =>
      /<string name="orientation" translatable="false">portrait<\/string>/m.test(
        content
      ),
...
function runChecks(checks, mode) {
  for (const check of checks) {
    if (!fs.existsSync(check.file)) {
      if (check.optional) {
        console.log(
          `[android-orientation-verify] Skip optional check (${check.label}): ${check.file}`
        );
        continue;
      }
    ...
    if (!check.test(content)) {
      fail(
        `Check failed (${check.label}). Expected ${check.expected} in ${check.file}`
      );
    }
  }
}
