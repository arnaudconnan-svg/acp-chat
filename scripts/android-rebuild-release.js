...
function removeAndroidProject() {
  if (!fs.existsSync(PROJECT_DIR)) {
    console.log(
      '[android-rebuild] android-project does not exist. Skipping cleanup.'
    );
    return;
  }

  console.log(
    '[android-rebuild] Removing android-project to avoid stale local artifacts...'
  );
...
function main() {
  ...
  console.log(
    '[android-rebuild] Regenerating Android project from twa-manifest...'
  );
  runNpm(['run', 'twa:build']);

  console.log(
    '[android-rebuild] Deploying release APK from clean Android state...'
  );
