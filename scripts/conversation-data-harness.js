...
assert(
  'isOpeningGreetingMessage: normal message → false',
  isOpeningGreetingMessage("J'ai mal dormi cette nuit.") === false
);

assert(
  'isOpeningGreetingMessage: empty string → false',
...
assert(
  "defaultMemory: contains 'Contexte stable'",
  mem.includes('Contexte stable (session)')
);
assert(
  "defaultMemory: contains 'Mouvements en cours'",
  mem.includes('Mouvements en cours')
);
assert(
  "defaultMemory: does not contain 'Signaux du tour precedent'",
  !mem.includes('Signaux du tour precedent') &&
    !mem.includes('Signaux du tour pr\u00e9c\u00e9dent')
);

// ── normalizeStoredFlags ─────────────────────────────────────────────────────

assert(
  'normalizeStoredFlags: valid object → returned as-is',
  JSON.stringify(normalizeStoredFlags({ foo: true })) ===
    JSON.stringify({ foo: true })
);

assert(
  'normalizeStoredFlags: null → {}',
  JSON.stringify(normalizeStoredFlags(null)) === '{}'
);

assert(
  'normalizeStoredFlags: array → {}',
  JSON.stringify(normalizeStoredFlags([])) === '{}'
);

assert(
  'normalizeStoredFlags: string → {}',
...
assert(
  'normalizeStoredStateSnapshot: array → null',
  normalizeStoredStateSnapshot([]) === null
);

{
  const snap = normalizeStoredStateSnapshot({
    memory: 'ma mémoire',
    flags: { f: 1 }
  });
  assert(
    'normalizeStoredStateSnapshot: valid → memory preserved',
    snap && snap.memory === 'ma mémoire'
  );
  assert(
    'normalizeStoredStateSnapshot: valid → flags preserved',
    snap && snap.flags.f === 1
  );
}

{
  ...
  assert(
    'normalizeStoredStateSnapshot: empty memory → defaultMemory',
...

{
  ...
  assert(
    'buildSafeConversationData: undefined → messages is []',
    JSON.stringify(d.messages) === '[]'
  );
  assert(
    'buildSafeConversationData: undefined → memory is defaultMemory',
    d.memory === defaultMemory()
  );
  assert(
    'buildSafeConversationData: undefined → flags is {}',
    JSON.stringify(d.flags) === '{}'
  );
  assert(
    'buildSafeConversationData: undefined → updatedAt is number',
    typeof d.updatedAt === 'number'
  );
  assert(
    'buildSafeConversationData: undefined → isPrivate false',
    d.isPrivate === false
  );
}

{
  ...
  assert(
    'buildSafeConversationData: custom memory preserved',
    d.memory === 'custom mem'
  );
  assert('buildSafeConversationData: isPrivate true', d.isPrivate === true);
  assert(
    'buildSafeConversationData: updatedAt preserved',
    d.updatedAt === 1000
  );
}
...

{
  ...
  assert(
    'countMeaningful: greeting not counted',
    countMeaningfulConversationMessages(msgs) === 2
  );
}

assert(
  'countMeaningful: empty array → 0',
  countMeaningfulConversationMessages([]) === 0
);
assert(
  'countMeaningful: null → 0',
  countMeaningfulConversationMessages(null) === 0
);

{
  const allGreetings = OPENING_GREETINGS.map((g) => ({
    role: 'assistant',
    content: g
  }));
  assert(
    'countMeaningful: all greetings → 0',
    countMeaningfulConversationMessages(allGreetings) === 0
  );
}
...

{
  ...
  assert(
    'selectPreferred: richer primary wins over newer empty fallback',
...

{
  ...
  const result = selectPreferredConversationData(primary, fallback);
  assert(
    'selectPreferred: richer fallback beats emptier primary',
    result.messages.length === 2
  );
}

{
  ...
  const result = selectPreferredConversationData(primary, fallback);
  assert(
    'selectPreferred: equal count, newer fallback wins',
...

{
  ...
  const result = selectPreferredConversationData(primary, fallback);
  assert(
    'selectPreferred: equal everything → primary returned',
...

{
  ...
  assert(
    'selectPreferred: both null → returns safe object',
...
if (failed === 0) {
  ...
  console.log(
    `conversation-data-harness: ${passed}/${passed + failed} passed, ${failed} FAILED`
  );
  process.exit(1);
}
