...
check(
  'Priority ordering: suicide_n2 < acute_crisis_followup < suicide_clarification',
  () => {
    const n2 = CHAT_PRIORITY_RULES.find((r) => r.id === 'suicide_n2');
    const followup = CHAT_PRIORITY_RULES.find(
      (r) => r.id === 'acute_crisis_followup'
    );
    const clarif = CHAT_PRIORITY_RULES.find(
      (r) => r.id === 'suicide_clarification'
    );
    assert(
      n2.priority < followup.priority,
      'suicide_n2 should have lower priority number than acute_crisis_followup'
    );
    assert(
      followup.priority < clarif.priority,
      'acute_crisis_followup should have lower priority number than suicide_clarification'
    );
  }
);
...
check('Priority ordering: recall_long_term < recall_none', () => {
  ...
  assert(
    longTerm.priority < none.priority,
    'recall_long_term should have lower priority number than recall_none'
  );
});
...
check('CHAT_PRIORITY_MATCHERS has an entry for each rule id', () => {
  for (const rule of CHAT_PRIORITY_RULES) {
    assert(
      typeof CHAT_PRIORITY_MATCHERS[rule.id] === 'function',
      `no matcher for rule ${rule.id}`
    );
  }
});
...
check(
  'acute_crisis_followup: matches when acuteCrisis=true and crisisResolved!=true',
  () => {
    const result = resolveChatPriorityRule({
      phase: 'post_suicide',
    ...
    assert(
      result?.id === 'acute_crisis_followup',
      `expected acute_crisis_followup, got ${result?.id}`
    );
  }
);
...
check('acute_crisis_followup: does NOT match when crisisResolved=true', () => {
  ...
  assert(
    result?.id !== 'acute_crisis_followup',
    `acute_crisis_followup should not match when crisisResolved=true`
  );
});
...
check('suicide_clarification: matches when suicideLevel=N1', () => {
  ...
  assert(
    result?.id === 'suicide_clarification',
    `expected suicide_clarification, got ${result?.id}`
  );
});

check(
  'suicide_clarification: matches when needsClarification=true regardless of level',
  () => {
    const result = resolveChatPriorityRule({
      phase: 'post_suicide',
...
check(
  'acuteCrisis takes priority over N1 (acute_crisis_followup before suicide_clarification)',
  () => {
    const result = resolveChatPriorityRule({
      phase: 'post_suicide',
...
check('recall_long_term: matches when isLongTermMemoryRecall=true', () => {
  ...
  assert(
    result?.id === 'recall_long_term',
    `expected recall_long_term, got ${result?.id}`
  );
});

check(
  'recall_none: matches when isRecallAttempt=true and calledMemory=none',
  () => {
    const result = resolveChatPriorityRule({
      phase: 'post_recall',
    ...
    assert(
      result?.id === 'recall_none',
      `expected recall_none, got ${result?.id}`
    );
  }
);
...
check(
  'recall_long_term takes priority over recall_none when both would match',
  () => {
    // isLongTermMemoryRecall=true + isRecallAttempt=true + calledMemory=none
    const result = resolveChatPriorityRule({
      phase: 'post_recall',
      recallRouting: {
        isLongTermMemoryRecall: true,
