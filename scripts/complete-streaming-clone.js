...
console.log(
  `[1] Extracted /chat: ${chatStart}-${chatEnd} (${chatEnd - chatStart} bytes)`
);
...
console.log(
  `[2] Found /chat/stream stub at ${streamStart}, app.listen at ${listenStart}`
);
...
// 3a. Change route
cloned = cloned.replace('app.post("/chat"', 'app.post("/chat/stream"');
...
// 3c. Change scope
cloned = cloned.replace('scope: "chat",', 'scope: "chat_stream",');
...
console.log(
  `[4] Updated server.js - cloned /chat to /chat/stream with SSE adaptation`
);
console.log(
  `[OK] Complete! server.js now has full /chat/stream implementation`
);
