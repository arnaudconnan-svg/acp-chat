...
function runBestEffort(command, args, options = {}) {
  ...
  if (result.status !== 0) {
    const details = String(result.stderr || result.stdout || '').trim();
    console.warn(
      `[android-deploy] Non-blocking command failed: ${command} ${args.join(' ')}${details ? ` (${details})` : ''}`
    );
  }
...
function getLatestBuildToolsDir(sdkRoot) {
  ...
  const versions = fs
    ...
    .filter((entry) =>
      fs.statSync(path.join(buildToolsDir, entry)).isDirectory()
    )
    .sort((a, b) =>
...
function main() {
  ...
  const apksigner =
    process.platform === 'win32'
      ? path.join(buildToolsDir, 'apksigner.bat')
      : path.join(buildToolsDir, 'apksigner');
  const zipalign =
    process.platform === 'win32'
      ? path.join(buildToolsDir, 'zipalign.exe')
      : path.join(buildToolsDir, 'zipalign');
  const adb =
    process.platform === 'win32'
  ...
  const keystorePass = String(
    process.env.FACILITAT_KEYSTORE_PASS || 'facilitat123'
  );
  ...
  if (install.status !== 0) {
    ...
    if (
      /INSTALL_FAILED_UPDATE_INCOMPATIBLE|signatures do not match/i.test(
        combined
      )
    ) {
      console.log(
        `[android-deploy] Existing ${PACKAGE_NAME} has a different signature. Uninstalling before reinstall.`
      );
  ...
  console.log(
    `[android-deploy] Restarting ${PACKAGE_NAME} to ensure fresh runtime...`
  );
  runBestEffort(adb, ['shell', 'am', 'force-stop', BROWSER_HOST_PACKAGE], {
    stdio: 'inherit'
  });
  run(
    adb,
