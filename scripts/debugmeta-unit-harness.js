...
// 1. buildTopChips
console.log('\n-- buildTopChips');
assert('N2 urgence chip', buildTopChips({ suicideLevel: 'N2' }), [
  'URGENCE : risque suicidaire'
]);
assert('N1 clarification chip', buildTopChips({ suicideLevel: 'N1' }), [
  'Risque suicidaire à clarifier'
]);
assert(
  'exploration_open EXPLORATION',
  buildTopChips({ conversationState: 'exploration_open' }),
  ['EXPLORATION']
);
assert(
  'exploration_open interpretation submode',
  buildTopChips({
...
assert(
  'exploration_open phenomenological_follow',
  buildTopChips({
    conversationState: 'exploration_open',
    explorationSignal: 'phenomenological_follow'
  }),
  ['EXPLORATION : accompagnement']
);
assert('info_pure', buildTopChips({ conversationState: 'info_pure' }), [
  'INFO PURE'
]);
assert('info_features', buildTopChips({ conversationState: 'info_features' }), [
  'INFO APP : fonctionnalités'
]);
assert(
  'info_psychoeducation',
  buildTopChips({ conversationState: 'info_psychoeducation' }),
  ['PSYCHOEDUCATION']
);
assert(
  'discharge_regulated',
  buildTopChips({ conversationState: 'discharge_regulated' }),
  ['DECHARGE : régulée']
);
assert(
  'discharge_dysregulated',
  buildTopChips({ conversationState: 'discharge_dysregulated' }),
  ['DECHARGE : dérégulée']
);
assert(
  'N0 no state empty',
  buildTopChips({ suicideLevel: 'N0', conversationState: null }),
  []
);
assert(
  'N2 overrides state',
  buildTopChips({ suicideLevel: 'N2', conversationState: 'exploration_open' }),
  ['URGENCE : risque suicidaire']
);
assert(
  'interpretationRejection chip',
  buildTopChips({ interpretationRejection: true }),
  ["Rejet d'interprétation"]
);
assert('isRecallRequest chip', buildTopChips({ isRecallRequest: true }), [
  'Demande de rappel mémoire'
]);
assert(
  'needsSoberReadjustment chip',
  buildTopChips({ needsSoberReadjustment: true }),
  ['Réajustement sobre']
);
assert(
  'relationalAdjustmentActive chip',
  buildTopChips({ relationalAdjustmentActive: true }),
  ['Ajustement relationnel']
);
assert(
  'exploration_open + recall + sober',
  buildTopChips({
    conversationState: 'exploration_open',
...
console.log('\n-- buildDirectivityText');
assert(
  'non-exploration state empty string',
  buildDirectivityText({ conversationState: 'info_features' }),
  ''
);
assert(
  'null state empty string',
  buildDirectivityText({ conversationState: null }),
  ''
);
assert(
  'exploration_open level 0 no calibration empty string',
  buildDirectivityText({
    conversationState: 'exploration_open',
...
assert(
  'exploration_open level 2 no calibration directivity line only',
  buildDirectivityText({
    conversationState: 'exploration_open',
...
const n2 = buildResponseDebugMeta({
  suicideLevel: 'N2',
...
assert('exploration topChips', explo.topChips, [
  'EXPLORATION : accompagnement'
]);
assert(
  'exploration conversationState',
  explo.conversationState,
  'exploration_open'
);
assert('exploration intent', explo.intent, 'accompagner_sans_guider');
assert('exploration forbidden', explo.forbidden, [
  'diagnostic',
  'conseil_direct'
]);
...
assert(
  ...
  explo.actionCollapseGuardActive,
  true
);
assert(
  'exploration explorationSignal',
  explo.explorationSignal,
  'phenomenological_follow'
);
assert(
  'exploration directivityText includes calibration',
  explo.directivityText.includes('Niveau de structuration retenu : 3/4'),
  true
);
assert('exploration pipelineStages length', explo.pipelineStages.length, 1);
assert(
  'exploration pipelineStages[0].stage',
  explo.pipelineStages[0].stage,
  'suicide_analysis'
);
assert(
  'exploration pipelineStages[0].deltaMs',
  explo.pipelineStages[0].deltaMs,
  42
);
assert('exploration requestId', explo.requestId, 'req-abc-123');
assert('exploration traceId', explo.traceId, 'trace-abc-123');
assert(
  'exploration explorationCalibrationLevel',
  explo.explorationCalibrationLevel,
  3
);
...
assert('info topChips (info_features)', info.topChips, [
  'INFO APP : fonctionnalités'
]);
...
const discharge = buildResponseDebugMeta({
...
assert(
  'recall isRecallRequest chip (with default exploration_open state)',
  recall.topChips,
  ['EXPLORATION', 'Demande de rappel mémoire']
);
...
assert(
  'pipelineStages length (nulls filtered)',
  piped.pipelineStages.length,
  3
);
assert('pipelineStages[0]', piped.pipelineStages[0], {
  stage: 'step_a',
  deltaMs: 10
});
assert('pipelineStages[1] bad deltaMs null', piped.pipelineStages[1], {
  stage: 'step_b',
  deltaMs: null
});
assert('pipelineStages[2]', piped.pipelineStages[2], {
  stage: 'step_c',
  deltaMs: 30
});
assert(
  'empty pipelineStages',
  buildResponseDebugMeta({ pipelineStages: [] }).pipelineStages,
  []
);
assert(
  'non-array pipelineStages []',
  buildResponseDebugMeta({ pipelineStages: null }).pipelineStages,
  []
);
...
const withFallback = buildResponseDebugMeta({
  memory: '',
  normalizeMemory: (m) => String(m || '').trim() || 'NONE'
});
...
const withBeforeSanitization = buildResponseDebugMeta({
  memoryBeforeSanitization: '  Old text.  ',
  normalizeMemory: (m) => String(m || '').trim()
});
assert(
  'memoryBeforeSanitization present',
  withBeforeSanitization.memoryBeforeSanitization,
  'Old text.'
);
const withoutBeforeSanitization = buildResponseDebugMeta({
  memoryBeforeSanitization: ''
});
assert(
  'memoryBeforeSanitization null when empty',
  withoutBeforeSanitization.memoryBeforeSanitization,
  null
);
...
const postCrisis = buildResponseDebugMeta({
  ...
  emergencySupportText:
    'urgences : 15 (SAMU) ou 112 | prevention suicide : 3114'
});
assert('postCrisisSupportActive', postCrisis.postCrisisSupportActive, true);
assert(
  'postCrisisSupportCarryTurn',
  postCrisis.postCrisisSupportCarryTurn,
  true
);
assert(
  'emergencySupportText',
  postCrisis.emergencySupportText,
  'urgences : 15 (SAMU) ou 112 | prevention suicide : 3114'
);
...
assert(
  'exploration_open state',
  buildResponseDebugMeta({ conversationState: 'exploration_open' })
    .conversationState,
  'exploration_open'
);
assert(
  'unknown state defaults to exploration_open',
  buildResponseDebugMeta({ conversationState: 'unknown_state' })
    .conversationState,
  'exploration_open'
);
assert(
  'null state defaults to exploration_open',
  buildResponseDebugMeta({ conversationState: null }).conversationState,
  'exploration_open'
);
...
assert(
  'stateTransitionValid default true',
  stDefault.stateTransitionValid,
  true
);
assert(
  'stateTransitionRequested default null',
  stDefault.stateTransitionRequested,
  null
);
const stValid = buildResponseDebugMeta({
  stateTransitionFrom: 'exploration_open',
  stateTransitionValid: true,
  stateTransitionRequested: null
});
assert(
  'stateTransitionFrom string pass-through',
  stValid.stateTransitionFrom,
  'exploration_open'
);
...
assert(
  'secondaryTension family alias relationalRupture',
  stAlias.secondaryTension?.family,
  'alliance_rupture'
);
assert(
  'secondaryTension confidence alias strong',
  stAlias.secondaryTension?.confidence,
  'high'
);
...
assert(
  'secondaryTension unknown family -> null',
  stUnknown.secondaryTension,
  null
);
...
console.log('\n' + '='.repeat(60));
console.log(
  'debugmeta-unit-harness: ' + passed + ' passed, ' + failed + ' failed'
);
