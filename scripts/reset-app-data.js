...
async function main() {
  ...
  const localResets = [
    {
      path: path.join(__dirname, '..', 'data', 'messages.json'),
      kind: 'array'
    },
    {
      path: path.join(__dirname, '..', 'data', 'conversations.json'),
      kind: 'array'
    },
  ...
  console.log(
    `[RESET-DATA] Local files: ${localResets.map((entry) => entry.path).join(', ')}`
  );

  if (!apply) {
    console.log(
      '[RESET-DATA] Dry run only. Re-run with --apply to delete Firebase data and reset local JSON files.'
    );
    return;
  }
  ...
  console.log(
    '[RESET-DATA] Firebase data removed and local JSON stores reset.'
  );
}
