...
function generateMipmap(dpi, size) {
  ...
  try {
    execSync(
      `magick convert "${SOURCE_ICON}" -resize ${size}x${size} "${outFile}"`,
      {
        stdio: 'pipe',
        shell: true
      }
    ...
    console.log(
      `Generated: mipmap-${dpi}/ic_maskable.png (${size_bytes} bytes)`
    );
  } catch (err) {
...
function main() {
  ...
  console.log(
    `[android-icon-generate] Regenerating mipmap from ${path.basename(SOURCE_ICON)}...`
  );
