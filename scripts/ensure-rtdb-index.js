...
function ensureMessagesConversationIndex(rulesContainer) {
  ...
  if (
    !next.rules.messages ||
    typeof next.rules.messages !== 'object' ||
    Array.isArray(next.rules.messages)
  ) {
    next.rules.messages = {};
  }
...
function ensureUsersEmailIndex(rulesContainer) {
  ...
  if (
    !next.rules.users ||
    typeof next.rules.users !== 'object' ||
    Array.isArray(next.rules.users)
  ) {
    next.rules.users = {};
  }
...
async function main() {
  ...
  console.log(
    `[RTDB-INDEX] Existing messages index present: ${hasRequiredIndex(rulesContainer)}`
  );
  console.log(
    `[RTDB-INDEX] Existing users index present: ${hasRequiredUsersIndex(rulesContainer)}`
  );

  if (!changed) {
    console.log(
      '[RTDB-INDEX] No change needed. Required indexes are already present.'
    );
    return;
  }
  ...
  if (!apply) {
    console.log(
      '[RTDB-INDEX] Dry run only. Re-run with --apply to push updated rules.'
    );
    return;
  }
  ...
  const backupFile = path.join(
    backupDir,
    `rtdb-rules-backup-${Date.now()}.json`
  );
  ...
  if (!hasRequiredIndex(refreshedContainer)) {
    throw new Error(
      'Post-update validation failed: conversationId index not found in rules'
    );
  }

  if (!hasRequiredUsersIndex(refreshedContainer)) {
    throw new Error(
      'Post-update validation failed: email index not found in rules'
    );
  }

  console.log(
    '[RTDB-INDEX] Validation OK. messages and users indexes are present.'
  );
}
