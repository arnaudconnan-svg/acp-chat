...
check('anchor found: uses DB slice and no fallback', () => {
  ...
  assert(
    out.seededMessages.length === 2,
    `expected 2 seeded messages, got ${out.seededMessages.length}`
  );
  assert(
    out.resolvedAnchorMessageId === 'm2',
    `expected m2, got ${out.resolvedAnchorMessageId}`
  );
});
...
check('anchor missing + seed provided: uses seed fallback', () => {
  ...
  assert(
    out.resolvedAnchorMessageId === 's2',
    `expected s2, got ${out.resolvedAnchorMessageId}`
  );
});
...
check('anchor missing + empty seed: returns anchor_not_found', () => {
  ...
  assert(
    ...
    `expected anchor_not_found, got ${out.error}`
  );
  assert(
    out.seededMessages.length === 0,
    `expected 0 seeded messages, got ${out.seededMessages.length}`
  );
});

check(
  'no anchor + seed: accepted and anchor inferred from latest seed id',
  () => {
    const out = resolveBranchSeedPayload({
      messageEntries,
      anchorMessageId: '',
    ...
    assert(
      out.resolvedAnchorMessageId === 's9',
      `expected s9, got ${out.resolvedAnchorMessageId}`
    );
  }
);
...
check('seed normalization filters invalid entries', () => {
  ...
  assert(
    out.seededMessages.length === 1,
    `expected 1 seeded message, got ${out.seededMessages.length}`
  );
});
