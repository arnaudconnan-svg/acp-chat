...
function readTwaManifest() {
  ...
  if (!fs.existsSync(manifestPath)) {
    console.error(
      "[bubblewrap] twa-manifest.json not found. Run 'npm run twa:manifest' first."
    );
    process.exit(1);
  }
  ...
  if (
    !manifest ||
    typeof manifest.webManifestUrl !== 'string' ||
    !manifest.webManifestUrl.trim()
  ) {
    console.error(
      '[bubblewrap] Invalid twa-manifest.json: missing webManifestUrl.'
    );
    process.exit(1);
  }
...
function resolveSdkRoot() {
  const candidates = [
    ...
    path.join(
      String(process.env.USERPROFILE || process.env.HOME || '').trim(),
      '.bubblewrap',
      'android_sdk'
    )
  ].filter(Boolean);
...
function writeLocalProperties(projectDir) {
  ...
  if (!sdkRoot) {
    console.warn(
      '[bubblewrap] Android SDK not found; skipping local.properties generation.'
    );
    return;
  }
  ...
  console.log(
    '[bubblewrap] Wrote local.properties with resolved Android SDK path.'
  );
}
...
function buildBubblewrapAnswers(manifest) {
  ...
  const launcherName =
    String(manifest.launcherName || appName).trim() || appName;
  const versionCode = Number.isFinite(Number(manifest.appVersionCode))
    ? String(Number(manifest.appVersionCode))
    : '1';
  const display =
    String(manifest.display || 'standalone').trim() || 'standalone';
  const orientation =
    String(manifest.orientation || 'portrait').trim() || 'portrait';
  const themeColor =
    String(manifest.themeColor || '#F4F9F9').trim() || '#F4F9F9';
  const backgroundColor =
    String(manifest.backgroundColor || themeColor).trim() || themeColor;
  ...
  const keystorePath = path.join(
    String(process.env.USERPROFILE || process.env.HOME || ''),
    '.android',
    'facilitat-dev.keystore'
  );
  const keyAlias =
    String(process.env.FACILITAT_KEY_ALIAS || 'facilitat-dev').trim() ||
    'facilitat-dev';

  return (
    [
      'y',
      String(manifest.host || 'acp-chat-beta.onrender.com').trim() ||
        'acp-chat-beta.onrender.com',
      startPath,
      appName,
      launcherName,
      packageId,
...
function installBubblewrap() {
  ...
  try {
    ...
    console.error(
      '[bubblewrap] Try installing manually: npm install -g @bubblewrap/cli'
    );
    process.exit(1);
  }
}
...
function main() {
  ...
  console.log('');
  console.log(
    '3. Apply persistent Android customizations (country picker styles, etc.):'
  );
