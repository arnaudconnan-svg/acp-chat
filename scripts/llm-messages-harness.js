...
// --- Cas 2 : historique sans doublon
{
  ...
  assert(
    'normal history → last content is new message',
    turns[turns.length - 1].content === 'Deuxième message'
  );
  assert(
    'normal history → no consecutive user turns',
    countConsecutiveDuplicateUserTurns(turns) === 0
  );
}
...
// --- Cas 3 : historique qui se termine déjà par le même message user (le bug original)
{
  ...
  assert(
    'history ends with same user msg → no duplicate added',
    turns.length === 2
  );
  assert(
    'history ends with same user msg → no consecutive user turns',
    countConsecutiveDuplicateUserTurns(turns) === 0
  );
  assert(
    'history ends with same user msg → last content unchanged',
    turns[turns.length - 1].content === 'Message répété'
  );
}

// --- Cas 4 : même message mais whitespace différent → dédupliqué aussi
{
  const history = [{ role: 'user', content: '  Avec espaces  ' }];
...
// --- Cas 5 : contenu similaire mais rôle différent → PAS de déduplication (c'est le bot qui a parlé en dernier)
{
  ...
  assert(
    'same content but different role → user turn added normally',
    turns.length === 3
  );
  assert(
    'same content but different role → last role is user',
    lastRole(turns) === 'user'
  );
}
...
// --- Cas 7 : message vide
{
  ...
  assert(
    'empty message → 1 user turn with empty content',
    turns.length === 1 && turns[0].content === ''
  );
}
...
// --- Cas 8 : historique long avec le current message au milieu (pas la fin) → ajouté normalement
{
  ...
  assert(
    'current msg matches older entry but not last → still appended',
    turns.length === 5
  );
  assert(
    'current msg matches older entry → no consecutive user turns',
    countConsecutiveDuplicateUserTurns(turns) === 0
  );
}
...
if (failed === 0) {
  ...
  console.log(
    `llm-messages-harness: ${passed}/${passed + failed} passed, ${failed} FAILED`
  );
  process.exit(1);
}
