...
check('Scenario 1: envelope full', () => {
  ...
  assert(
    isEqual(computeGaugeSegments(state.monthly.remaining, { capacity: 100 }), [
      'dark',
      'dark',
      'dark',
      'dark'
...
check('Scenario 3: low envelope no reserve', () => {
  ...
  const segments = computeGaugeSegments(10, {
    capacity: 100,
    showRedWhenLow: true
  });
  assert(
...
check('Scenario 4: low envelope with reserve', () => {
  ...
  const warning = shouldShowLowEnvelopeWarning(raw);
  const monthlySegments = computeGaugeSegments(10, {
    capacity: 100,
...
check('Scenario 8: monthly renewal', () => {
  ...
  assert(
    renewal.state.monthly.remaining === 100,
    'monthly should reset to 100'
  );
