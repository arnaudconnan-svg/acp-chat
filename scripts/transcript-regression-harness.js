...
const casesPath = path.join(
  __dirname,
  'fixtures',
  'transcript-regression-cases.json'
);
...
for (const testCase of regressionCases) {
  ...
  assert(
    `${labelPrefix} expected length`,
    turns.length === testCase.expectedLength,
    `got ${turns.length}`
  );
  assert(
    `${labelPrefix} expected last role`,
    turns.length > 0 &&
      turns[turns.length - 1].role === testCase.expectedLastRole,
...
if (failed === 0) {
  ...
  console.log(
    `[TRANSCRIPT] ${passed}/${passed + failed} passed, ${failed} failed`
  );
  process.exit(1);
}
