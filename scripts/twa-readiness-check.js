...
function main() {
  ...
  allGood =
    check(
      typeof manifest.short_name === 'string' &&
        manifest.short_name.trim().length > 0,
      'manifest short_name set',
  ...
  allGood =
    check(
      ...
      'manifest start_url starts with /',
      'manifest start_url invalid'
    ) && allGood;
  allGood =
    check(
      typeof manifest.scope === 'string' && manifest.scope.startsWith('/'),
      'manifest scope starts with /',
      'manifest scope invalid'
    ) && allGood;
  allGood =
    check(
      typeof manifest.id === 'string' && manifest.id.startsWith('/'),
      'manifest id starts with /',
      'manifest id invalid'
    ) && allGood;
  allGood =
    check(
      manifest.display === 'standalone' || manifest.display === 'fullscreen',
      'manifest display suitable for install',
      'manifest display should be standalone/fullscreen'
    ) && allGood;
  allGood =
    check(
  ...
  allGood =
    check(
      ...
      'manifest background_color missing'
    ) && allGood;
  ...
  allGood =
    check(
      Boolean(icon192),
      'manifest has 192x192 icon entry',
      'manifest missing 192x192 icon'
    ) && allGood;
  allGood =
    check(
      Boolean(icon512),
      'manifest has 512x512 icon entry',
  ...
  if (icon192 && typeof icon192.src === 'string') {
    allGood =
      check(
        fs.existsSync(path.join(publicDir, icon192.src.replace(/^\//, ''))),
        'icon 192 file exists',
        `icon 192 file missing at ${icon192.src}`
      ) && allGood;
  }
  ...
  if (icon512 && typeof icon512.src === 'string') {
    allGood =
      check(
        fs.existsSync(path.join(publicDir, icon512.src.replace(/^\//, ''))),
        'icon 512 file exists',
        `icon 512 file missing at ${icon512.src}`
      ) && allGood;
  }
  ...
  if (fs.existsSync(indexPath)) {
    ...
    const hasDisabledAlphaPolicyMarker =
      indexHtml.includes('__FACILITAT_SW_POLICY__') &&
      indexHtml.includes('disabled_alpha');
    allGood =
      check(
  ...
  allGood =
    check(
      ...
      'assetlinks file present',
      'public/.well-known/assetlinks.json missing'
    ) && allGood;
    ...
    try {
      ...
      allGood =
        check(
          ...
          'assetlinks contains a JSON array',
          'assetlinks must be a JSON array'
        ) && allGood;
    } catch (err) {
      allGood =
        check(false, '', `assetlinks invalid JSON: ${err.message}`) && allGood;
    }
  }
