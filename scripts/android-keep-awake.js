...
function runShell(adbPath, shellCommand) {
  ...
  if (result.status !== 0) {
    fail(
      `adb shell ${shellCommand.join(' ')} failed. stderr: ${String(result.stderr || '').trim()}`
    );
  }
  return result;
}
...
function printStatus(adbPath) {
  ...
  console.log(
    `[android:keep-awake] stay_on_while_plugged_in=${rawValue || '(empty)'}`
  );
  console.log(
    enabled
...
function enableKeepAwake(adbPath) {
  ...
  console.log(
    '[android:keep-awake] Enabled. Device should stay awake while plugged in and connected over adb.'
  );
  printStatus(adbPath);
}
...
function disableKeepAwake(adbPath) {
  ...
  console.log(
    '[android:keep-awake] Disabled. Device can sleep normally again.'
  );
  printStatus(adbPath);
}
