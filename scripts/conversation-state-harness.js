...
check('resolveConversationState keeps discharge priority', () => {
  assert(
    resolveConversationState({ detectedState: 'discharge_regulated' }) ===
      'discharge_regulated'
  );
  assert(
    resolveConversationState({ detectedState: 'discharge_dysregulated' }) ===
...
check('info states pass through', () => {
  assert(
    resolveConversationState({ detectedState: 'info_pure' }) === 'info_pure'
  );
  assert(
    resolveConversationState({ detectedState: 'info_features' }) ===
      'info_features'
  );
  assert(
    resolveConversationState({ detectedState: 'info_psychoeducation' }) ===
...
check('dependency care pending overrides exploration and info', () => {
  assert(
    resolveConversationState({
      detectedState: 'exploration',
      dependencyCareMessagePending: true
    }) === 'need_human_support'
  );
  assert(
    resolveConversationState({
...
check(
  'dependency care pending keeps support state through alliance rupture',
  () => {
    assert(
      resolveConversationState({
        detectedState: 'exploration',
...
check('isValidTransition supports need_human_support transitions', () => {
  assert(
    isValidTransition('need_human_support', 'need_human_support') === true
  );
...
check('state tables contain no contact key', () => {
  assert(
    !('contact' in STATE_FORBIDDEN),
    'STATE_FORBIDDEN.contact should not exist'
  );
