...
for (const s of EXPECTED_STATES) {
  assert(
    CONVERSATION_STATES.includes(s),
    `CONVERSATION_STATES includes '${s}'`
  );
}
...
for (const s of CONVERSATION_STATES) {
  assert(
    Array.isArray(STATE_TRANSITIONS[s]),
    `STATE_TRANSITIONS has entry for '${s}'`
  );
  assert(
    STATE_TRANSITIONS[s].length > 0,
    `STATE_TRANSITIONS['${s}'] is non-empty`
  );
}
...
// Known valid edges
assert(
  isValidTransition('exploration', 'discharge'),
  'isValidTransition: exploration -> discharge'
);
assert(
  isValidTransition('discharge', 'exploration'),
  'isValidTransition: discharge -> exploration'
);
// Self-transitions
assert(
  isValidTransition('exploration', 'exploration'),
  'isValidTransition: exploration -> exploration (self)'
);
assert(
  isValidTransition('discharge', 'discharge'),
  'isValidTransition: discharge -> discharge (self)'
);
assert(
  isValidTransition('info', 'info'),
  'isValidTransition: info -> info (self)'
);
// closure has restricted successors
assert(
  !isValidTransition('closure', 'alliance_rupture'),
  'isValidTransition: closure -> alliance_rupture BLOCKED'
);
// unknown state is permissive
assert(
  isValidTransition('unknown_legacy_state', 'exploration'),
  'isValidTransition: unknown legacy state is permissive'
);
...
assert(
  resolve({ detectedState: 'info_features' }) === 'info_features',
  'resolve: info_features -> info_features'
);
assert(
  resolve({ detectedState: 'info_pure' }) === 'info_pure',
  'resolve: info_pure -> info_pure'
);
assert(
  resolve({ detectedState: 'info_psychoeducation' }) === 'info_psychoeducation',
  'resolve: info_psychoeducation -> info_psychoeducation'
);

// Default: exploration_open
assert(
  resolve() === 'exploration_open',
  'resolve: default is exploration_open'
);

// Default with restrained: exploration_restrained
assert(
  resolve({ directivityLevel: 2 }) === 'exploration_open',
  'resolve: directivityLevel=2 keeps exploration_open'
);
assert(
  resolve({ directivityLevel: 3 }) === 'exploration_restrained',
  'resolve: directivityLevel>=3 gives exploration_restrained'
);
...
assert(
  resolve({ closureIntent: true, allianceSignal: 'rupture' }) ===
    'alliance_rupture',
...
assert(
  baseStateOf('exploration_open') === 'exploration',
  'baseStateOf: exploration_open -> exploration'
);
assert(
  baseStateOf('exploration_restrained') === 'exploration',
  'baseStateOf: exploration_restrained -> exploration'
);
assert(
  baseStateOf('discharge_regulated') === 'discharge',
  'baseStateOf: discharge_regulated -> discharge'
);
assert(
  baseStateOf('discharge_dysregulated') === 'discharge',
  'baseStateOf: discharge_dysregulated -> discharge'
);
assert(baseStateOf('info_pure') === 'info', 'baseStateOf: info_pure -> info');
assert(
  baseStateOf('info_features') === 'info',
  'baseStateOf: info_features -> info'
);
assert(
  baseStateOf('info_psychoeducation') === 'info',
  'baseStateOf: info_psychoeducation -> info'
);
assert(
  baseStateOf('alliance_rupture') === 'alliance_rupture',
  'baseStateOf: alliance_rupture -> alliance_rupture'
);
assert(
  baseStateOf('need_human_support') === 'need_human_support',
  'baseStateOf: need_human_support passthrough'
);
assert(baseStateOf('closure') === 'closure', 'baseStateOf: closure -> closure');
assert(
  baseStateOf('n1_crisis') === 'n1_crisis',
  'baseStateOf: n1_crisis passthrough'
);
...
for (const st of allStates) {
  ...
  assert(
    Array.isArray(STATE_FORBIDDEN[st]),
    `STATE_FORBIDDEN['${st}'] is array`
  );
  assert(
    typeof STATE_INTENT[st] === 'string',
    `STATE_INTENT['${st}'] is string`
  );
}
